...
import (
	...
	"__GITHUB_HUBLIVE__mageutil"
	_ "__GITHUB_HUBLIVE__psrpc"
)

const (
	goChecksumFile = ".checksumgo"
	imageName      = "hublive/hublive-server"
)
...
func Build() error {
	...
	if err := mageutil.RunDir(context.Background(), "cmd/server", "go build -o ../../bin/hublive-server"); err != nil {
		return err
	}
...
func BuildLinux() error {
	...
	cmd := mageutil.CommandDir(context.Background(), "cmd/server", "go build -buildvcs=false -o ../../bin/hublive-server-" + buildArch)
	cmd.Env = []string{
		"GOOS=linux",
		"GOARCH=" + buildArch,
		"HOME=" + os.Getenv("HOME"),
