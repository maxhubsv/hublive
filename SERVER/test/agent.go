...
type agentClient struct {
	...
	requestedJobs chan *hublive.Job
...
func newAgentClient(token string, port uint32) (*agentClient, error) {
	...
	return &agentClient{
		conn:          conn,
		requestedJobs: make(chan *hublive.Job, 100),
		done:          make(chan struct{}),
	}, nil
}

func (c *agentClient) Run(jobType hublive.JobType, namespace string) (err error) {
	go c.read()

	switch jobType {
	case hublive.JobType_JT_ROOM:
		err = c.write(&hublive.WorkerMessage{
			Message: &hublive.WorkerMessage_Register{
				Register: &hublive.RegisterWorkerRequest{
	...
	case hublive.JobType_JT_PARTICIPANT:
		err = c.write(&hublive.WorkerMessage{
			Message: &hublive.WorkerMessage_Register{
				Register: &hublive.RegisterWorkerRequest{
					Type:      hublive.JobType_JT_PARTICIPANT,
					Version:   "version",
					Namespace: &namespace,
				},
			},
...
func (c *agentClient) handleAssignment(req *hublive.JobAssignment) {
	switch req.Job.Type {
	case hublive.JobType_JT_ROOM:
		c.roomJobs.Inc()
	case hublive.JobType_JT_PUBLISHER:
...
func (c *agentClient) handleAvailability(req *hublive.AvailabilityRequest) {
	switch req.Job.Type {
	case hublive.JobType_JT_ROOM:
	...
	c.write(&hublive.WorkerMessage{
		Message: &hublive.WorkerMessage_Availability{
			Availability: &hublive.AvailabilityResponse{
				JobId:     req.Job.Id,
				Available: true,
			},
		},
...
func (c *agentClient) handleRegister(req *hublive.RegisterWorkerResponse) {
	c.registered.Inc()
}

func (c *agentClient) write(msg *hublive.WorkerMessage) error {
	c.mu.Lock()
	defer c.mu.Unlock()
