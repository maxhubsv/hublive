...
import (
	...
	"github.com/maxhubsv/hublive-server/test/client"
)
...
func TestMultiNodeRouting(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &hublive.CreateRoomRequest{
		Name: testRoom,
	})
...
func TestMultiNodeUpdateAttributes(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.UpdateParticipant(contextWithToken(adminRoomToken(testRoom)), &hublive.UpdateParticipantRequest{
				Room:     testRoom,
				Identity: "au1",
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			c2 := createRTCClient("c2", defaultServerPort, testRTCServicePath, &client.Options{
				SignalRequestInterceptor: func(msg *hublive.SignalRequest, next client.SignalRequestHandler) error {
					switch msg.Message.(type) {
					case *hublive.SignalRequest_Offer, *hublive.SignalRequest_Answer, *hublive.SignalRequest_Leave:
						return nil
					default:
						return next(msg)
					}
				},
				SignalResponseInterceptor: func(msg *hublive.SignalResponse, next client.SignalResponseHandler) error {
					switch msg.Message.(type) {
					case *hublive.SignalResponse_Offer, *hublive.SignalResponse_Answer:
						return nil
					default:
						return next(msg)
					}
