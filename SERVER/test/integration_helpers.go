...
var roomClient hublive.RoomService
...
func setupSingleNodeTest(name string) (*service.HubliveServer, func()) {
...
func setupMultiNodeTest(name string) (*service.HubliveServer, *service.HubliveServer, func()) {
	logger.Infow("----------------STARTING TEST----------------", "test", name)
...
func waitForServerToStart(s *service.HubliveServer) {
	// wait till ready
...
func createSingleNodeServer(configUpdater func(*config.Config)) *service.HubliveServer {
	var err error
	...
	currentNode.SetNodeID(hublive.NodeID(guid.New(nodeID1)))
	...
	roomClient = hublive.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", defaultServerPort), &http.Client{})
	return s
}

func createMultiNodeServer(nodeID string, port uint32) *service.HubliveServer {
	var err error
	conf, err := config.NewConfig("", true, nil, nil)
	if err != nil {
		panic(fmt.Sprintf("could not create config: %v", err))
	...
	currentNode.SetNodeID(hublive.NodeID(nodeID))
	...
	roomClient = hublive.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", port), &http.Client{})
	return s
}
