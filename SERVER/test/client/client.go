...
import (
	...
	"github.com/maxhubsv/hublive-server/pkg/rtc/transport/transportfakes"
	"github.com/maxhubsv/hublive-server/pkg/rtc/types"
	"github.com/maxhubsv/hublive-server/pkg/sfu/buffer"
)

type SignalRequestHandler func(msg *hublive.SignalRequest) error
type SignalRequestInterceptor func(msg *hublive.SignalRequest, next SignalRequestHandler) error
type SignalResponseHandler func(msg *hublive.SignalResponse) error
type SignalResponseInterceptor func(msg *hublive.SignalResponse, next SignalResponseHandler) error
...
type RTCClient struct {
	...
	subscribedTracks   map[hublive.ParticipantID][]*webrtc.TrackRemote
	localParticipant   *hublive.ParticipantInfo
	remoteParticipants map[hublive.ParticipantID]*hublive.ParticipantInfo
	...
	pendingPublishedTracks map[string]*hublive.TrackInfo
	...
	// map of HubLive.ParticipantID and last packet
	lastPackets   map[hublive.ParticipantID]*rtp.Packet
	bytesReceived map[hublive.ParticipantID]uint64

	subscriptionResponse atomic.Pointer[hublive.SubscriptionResponse]

	nextDataTrackHandle        atomic.Uint32
	pendingPublishedDataTracks map[uint16]*hublive.DataTrackInfo
	pendingDataTrackWriters    []TrackWriter
	subscribedDataTracks       map[hublive.ParticipantID]map[uint16]*DataTrackRemote
}
...
func NewWebSocketConn(host, token string, opts *Options) (*websocket.Conn, error) {
	...
	if opts != nil && opts.UseJoinRequestQueryParam {
		...
		connectionSettings := &hublive.ConnectionSettings{
			AutoSubscribe:          opts.AutoSubscribe,
			AutoSubscribeDataTrack: &opts.AutoSubscribeDataTrack,
		}

		joinRequest := &hublive.JoinRequest{
		...
		if marshalled, err := proto.Marshal(joinRequest); err == nil {
			wrapped := &hublive.WrappedJoinRequest{
		...
		if opts != nil {
			...
			if opts.ClientInfo != nil {
				...
				if opts.ClientInfo.Sdk != hublive.ClientInfo_UNKNOWN {
...
func NewRTCClient(conn *websocket.Conn, useSinglePeerConnection bool, opts *Options) (*RTCClient, error) {
	...
	c := &RTCClient{
		...
		pendingPublishedTracks:     make(map[string]*hublive.TrackInfo),
		subscribedTracks:           make(map[hublive.ParticipantID][]*webrtc.TrackRemote),
		remoteParticipants:         make(map[hublive.ParticipantID]*hublive.ParticipantInfo),
		me:                         &webrtc.MediaEngine{},
		lastPackets:                make(map[hublive.ParticipantID]*rtp.Packet),
		bytesReceived:              make(map[hublive.ParticipantID]uint64),
		pendingPublishedDataTracks: make(map[uint16]*hublive.DataTrackInfo),
		subscribedDataTracks:       make(map[hublive.ParticipantID]map[uint16]*DataTrackRemote),
	}
	...
	var codecs []*hublive.Codec
	for _, codec := range []*hublive.Codec{
		{
			Mime: "audio/opus",
		},
		{
	...
	publisherHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t hublive.SignalTarget) error {
		return c.SendIceCandidate(ic, hublive.SignalTarget_PUBLISHER)
	})
	...
	if !c.useSinglePeerConnection {
		...
		subscriberHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t hublive.SignalTarget) error {
			if ic == nil {
				return nil
			}
			return c.SendIceCandidate(ic, hublive.SignalTarget_SUBSCRIBER)
		})
		...
		subscriberHandler.OnAnswerCalls(func(answer webrtc.SessionDescription, answerId uint32, _midToTrackID map[string]string) error {
			...
			return c.SendRequest(&hublive.SignalRequest{
				Message: &hublive.SignalRequest_Answer{
...
func (c *RTCClient) ID() hublive.ParticipantID {
	return c.id
}
...
func (c *RTCClient) handleSignalResponse(res *hublive.SignalResponse) error {
	switch msg := res.Message.(type) {
	case *hublive.SignalResponse_Join:
		c.localParticipant = msg.Join.Participant
		c.id = hublive.ParticipantID(msg.Join.Participant.Sid)
		c.lock.Lock()
		for _, p := range msg.Join.OtherParticipants {
			c.remoteParticipants[hublive.ParticipantID(p.Sid)] = p
		}
		c.lock.Unlock()
	...
	case *hublive.SignalResponse_Answer:
		logger.Infow(
	...
	case *hublive.SignalResponse_Offer:
	...
	case *hublive.SignalResponse_Trickle:
		candidateInit, err := signalling.FromProtoTrickle(msg.Trickle)
		if err != nil {
			return err
		}
		if msg.Trickle.Target == hublive.SignalTarget_PUBLISHER {
			c.publisher.AddICECandidate(candidateInit)
		} else {
			c.subscriber.AddICECandidate(candidateInit)
		}

	case *hublive.SignalResponse_Update:
		c.lock.Lock()
		for _, p := range msg.Update.Participants {
			if hublive.ParticipantID(p.Sid) != c.id {
				if p.State != hublive.ParticipantInfo_DISCONNECTED {
					c.remoteParticipants[hublive.ParticipantID(p.Sid)] = p
				} else {
					delete(c.remoteParticipants, hublive.ParticipantID(p.Sid))
				}
			}
		}
		c.lock.Unlock()

	case *hublive.SignalResponse_TrackPublished:
		logger.Debugw(
			"track published",
			"participant", c.localParticipant.Identity,
	...
	case *hublive.SignalResponse_RefreshToken:
		c.lock.Lock()
		c.refreshToken = msg.RefreshToken
		c.lock.Unlock()

	case *hublive.SignalResponse_TrackUnpublished:
	...
	case *hublive.SignalResponse_Pong:
		c.pongReceivedAt.Store(msg.Pong)

	case *hublive.SignalResponse_SubscriptionResponse:
		c.subscriptionResponse.Store(msg.SubscriptionResponse)

	case *hublive.SignalResponse_MediaSectionsRequirement:
		logger.Infow(
	...
	case *hublive.SignalResponse_PublishDataTrackResponse:
		logger.Debugw(
	...
	case *hublive.SignalResponse_DataTrackSubscriberHandles:
		logger.Infow(
		...
		for handle, publishedDataTrack := range msg.DataTrackSubscriberHandles.SubHandles {
			publisherID := hublive.ParticipantID(publishedDataTrack.PublisherSid)
			...
			if tracks[uint16(handle)] == nil {
				tracks[uint16(handle)] = NewDataTrackRemote(
					hublive.ParticipantIdentity(publishedDataTrack.PublisherIdentity),
					hublive.ParticipantID(publishedDataTrack.PublisherSid),
					uint16(handle),
					hublive.TrackID(publishedDataTrack.TrackSid),
...
func (c *RTCClient) ReadResponse() (*hublive.SignalResponse, error) {
	for {
...
func (c *RTCClient) SubscribedTracks() map[hublive.ParticipantID][]*webrtc.TrackRemote {
	// create a copy of this
	c.lock.Lock()
	defer c.lock.Unlock()
	tracks := make(map[hublive.ParticipantID][]*webrtc.TrackRemote, len(c.subscribedTracks))
	maps.Copy(tracks, c.subscribedTracks)
	return tracks
}

func (c *RTCClient) SubscribedDataTracks() map[hublive.ParticipantID]map[uint16]*DataTrackRemote {
	// create a copy of this
	c.lock.Lock()
	defer c.lock.Unlock()
	tracks := make(map[hublive.ParticipantID]map[uint16]*DataTrackRemote, len(c.subscribedDataTracks))
...
func (c *RTCClient) RemoteParticipants() []*hublive.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
	return funk.Values(c.remoteParticipants).([]*hublive.ParticipantInfo)
}

func (c *RTCClient) GetRemoteParticipant(sid hublive.ParticipantID) *hublive.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *RTCClient) Stop() {
	...
	_ = c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_Leave{
			Leave: &hublive.LeaveRequest{
				Reason: hublive.DisconnectReason_CLIENT_INITIATED,
				Action: hublive.LeaveRequest_DISCONNECT,
			},
		},
	})
...
func (c *RTCClient) GetSubscriptionResponseAndClear() *hublive.SubscriptionResponse {
	return c.subscriptionResponse.Swap(nil)
}

func (c *RTCClient) SendPing() error {
	return c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_Ping{
			Ping: time.Now().UnixNano(),
		},
	})
}

func (c *RTCClient) SendRequest(msg *hublive.SignalRequest) error {
...
func (c *RTCClient) SendIceCandidate(ic *webrtc.ICECandidate, target hublive.SignalTarget) error {
	...
	return c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_Trickle{
...
func (c *RTCClient) SetAttributes(attrs map[string]string) error {
	return c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_UpdateMetadata{
			UpdateMetadata: &hublive.UpdateParticipantMetadata{
				Attributes: attrs,
			},
		},
	})
...
func (c *RTCClient) SendAddTrack(cid string, mimeType string, name string, trackType hublive.TrackType) error {
	return c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_AddTrack{
...
func (c *RTCClient) PublishData(data []byte, kind hublive.DataPacket_Kind) error {
...
func (c *RTCClient) PublishDataTrack() (writer TrackWriter, err error) {
	...
	if err = c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_PublishDataTrackRequest{
			PublishDataTrackRequest: &hublive.PublishDataTrackRequest{
...
func (c *RTCClient) handleDataMessage(kind hublive.DataPacket_Kind, data []byte) {
	...
	dp.Kind = kind
	if val, ok := dp.Value.(*hublive.DataPacket_User); ok {
...
func (c *RTCClient) handleMediaSectionsRequirement(mediaSectionsRequirement *hublive.MediaSectionsRequirement) {
...
func (c *RTCClient) onOffer(offer webrtc.SessionDescription, offerId uint32, midToTrackID map[string]string) error {
	...
	return c.SendRequest(&hublive.SignalRequest{
		Message: &hublive.SignalRequest_Offer{
...
func (c *RTCClient) processRemoteTrack(track *webrtc.TrackRemote) {
	...
	if trackID == "" {
		trackID = hublive.TrackID(track.ID())
	}
	c.lock.Lock()
