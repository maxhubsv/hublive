	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("scenarioDataPublish/testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			c2.OnDataReceived = func(data []byte, sid string) {
				if string(data) == payload && hublive.ParticipantID(sid) == c1.ID() {
					received.Store(true)
				}
			}

			require.NoError(t, c1.PublishData([]byte(payload), hublive.DataPacket_RELIABLE))
...
func scenarioJoinClosedRoom(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.DeleteRoom(contextWithToken(createRoomToken()), &hublive.DeleteRoomRequest{
				Room: testRoom,
			})
...
func closeNonRTCRoom(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(createCtx, &hublive.CreateRoomRequest{
		Name: testRoom,
	})
	require.NoError(t, err)

	_, err = roomClient.DeleteRoom(createCtx, &hublive.DeleteRoomRequest{
		Room: testRoom,
	})
	require.NoError(t, err)
}
...
func roomServiceListRoom(t *testing.T) {
	...
	t.Run("list all rooms", func(t *testing.T) {
		res, err := roomClient.ListRooms(listCtx, &hublive.ListRoomsRequest{})
		require.NoError(t, err)
