...
func TestMultiNodeUpdateRoomMetadata(t *testing.T) {
	...
	t.Run("when room is empty", func(t *testing.T) {
		...
		rm, err := roomClient.UpdateRoomMetadata(contextWithToken(adminRoomToken("emptyRoom")), &hublive.UpdateRoomMetadataRequest{
			Room:     "emptyRoom",
...
func TestMultiNodeRemoveParticipant(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err := roomClient.RemoveParticipant(ctx, &hublive.RoomParticipantIdentity{
				Room:     testRoom,
			...
			listRes, err := roomClient.ListParticipants(ctx, &hublive.ListParticipantsRequest{
				Room: testRoom,
			})
			require.NoError(t, err)
...
func TestMultiNodeUpdateParticipantMetadata(t *testing.T) {
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			ctx := contextWithToken(adminRoomToken(testRoom))
			res, err := roomClient.UpdateParticipant(ctx, &hublive.UpdateParticipantRequest{
				Room:     testRoom,
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			testutils.WithTimeout(t, func() string {
				res, err := roomClient.GetParticipant(ctx, &hublive.RoomParticipantIdentity{
					Room:     testRoom,
					Identity: identity,
				})
				require.NoError(t, err)
			...
			res, err := roomClient.MutePublishedTrack(ctx, &hublive.MuteRoomTrackRequest{
				Room:     testRoom,
				Identity: identity,
				TrackSid: trackIDs[0],
				Muted:    true,
