...
func setupServerWithWebhook() (server *service.HubliveServer, testServer *webhookTestServer, finishFunc func(), err error) {
...
type webhookTestServer struct {
	server   *http.Server
	events   map[string]*hublive.WebhookEvent
	lock     sync.Mutex
...
func newTestServer(addr string) *webhookTestServer {
	s := &webhookTestServer{
		events:   make(map[string]*hublive.WebhookEvent),
...
func (s *webhookTestServer) GetEvent(name string) *hublive.WebhookEvent {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *webhookTestServer) ClearEvents() {
	s.lock.Lock()
	s.events = make(map[string]*hublive.WebhookEvent)
	s.lock.Unlock()
}
