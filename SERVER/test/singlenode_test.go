...
import (
	...
	testclient "github.com/maxhubsv/hublive-server/test/client"
)
...
func TestSinglePublisher(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			audioTrack := funk.Find(remoteC1.Tracks, func(ti *hublive.TrackInfo) bool {
...
func TestSingleNodeUpdateParticipant(t *testing.T) {
	...
	t.Run("update nonexistent participant", func(t *testing.T) {
		_, err := roomClient.UpdateParticipant(adminCtx, &hublive.UpdateParticipantRequest{
			Room:     testRoom,
			Identity: "nonexistent",
			Permission: &hublive.ParticipantPermission{
				CanPublish: true,
			},
		})
		require.Error(t, err)
...
func TestSingleNodeDoubleSlash(t *testing.T) {
	...
	roomClient = hublive.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d/", s.HTTPPort()), &http.Client{})
	_, err := roomClient.ListRooms(contextWithToken(listRoomToken()), &hublive.ListRoomsRequest{})
	require.NoError(t, err)
}
...
func TestAutoCreate(t *testing.T) {
	...
	t.Run("join with explicit createRoom", func(t *testing.T) {
		...
		// explicitly create
		_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &hublive.CreateRoomRequest{Name: testRoom})
		require.NoError(t, err)
...
func TestSingleNodeUpdateSubscriptionPermissions(t *testing.T) {
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			_, err = roomClient.UpdateParticipant(ctx, &hublive.UpdateParticipantRequest{
				Room:     testRoom,
				Identity: "sub",
				Permission: &hublive.ParticipantPermission{
					CanSubscribe: true,
	...
	for _, testRTCServicePath := range testRTCServicePaths {
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			require.Eventually(t, func() bool {
				...
				require.Equal(t, hublive.SubscriptionError_SE_CODEC_UNSUPPORTED, sr.Err)
				return true
		...
		t.Run(fmt.Sprintf("testRTCServicePath=%s", testRTCServicePath.String()), func(t *testing.T) {
			...
			go func() {
				...
				for !stopWrite.Load() {
					...
					if err := pub.PublishData(buf, hublive.DataPacket_RELIABLE); err != nil {
