...
func initDataPacketStats(nodeID string, nodeType hublive.NodeType) {
...
func RecordDataPacketStream(h *hublive.DataStream_Header, destCount int) {
	streamType := "unknown"
	switch h.ContentHeader.(type) {
	case *hublive.DataStream_Header_TextHeader:
		streamType = "text"
	case *hublive.DataStream_Header_ByteHeader:
		streamType = "bytes"
	}
