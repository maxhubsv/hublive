...
func initPacketStats(nodeID string, nodeType hublive.NodeType) {
	promPacketTotal = prometheus.NewCounterVec(prometheus.CounterOpts{
		Namespace:   hubliveNamespace,
		Subsystem:   "packet",
		Name:        "total",
...
func RecordPacketOutOfOrder(country string, direction Direction, trackSource hublive.TrackSource, trackType hublive.TrackType, ooo, total uint32) {
	if total > 0 {
...
func RecordJitter(country string, direction Direction, trackSource hublive.TrackSource, trackType hublive.TrackType, jitter uint32) {
	if jitter > 0 {
		promJitter.WithLabelValues(string(direction), trackSource.String(), trackType.String(), country).Observe(float64(jitter))
	}
}

func RecordRTT(country string, direction Direction, trackSource hublive.TrackSource, trackType hublive.TrackType, rtt uint32) {
	if rtt > 0 {
