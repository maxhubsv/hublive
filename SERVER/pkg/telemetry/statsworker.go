...
import (
	...
	protoutils "__GITHUB_HUBLIVE__protocol/utils"
)
...
type StatsWorker struct {
	...
	lock             sync.RWMutex
	outgoingPerTrack map[hublive.TrackID][]*hublive.AnalyticsStat
	incomingPerTrack map[hublive.TrackID][]*hublive.AnalyticsStat
...
func newStatsWorker(
	...
	s := &StatsWorker{
		...
		outgoingPerTrack:    make(map[hublive.TrackID][]*hublive.AnalyticsStat),
		incomingPerTrack:    make(map[hublive.TrackID][]*hublive.AnalyticsStat),
	}
	s.refCount.Activate(guard)
	return s
}

func (s *StatsWorker) OnTrackStat(trackID hublive.TrackID, direction hublive.StreamType, stat *hublive.AnalyticsStat) {
	s.lock.Lock()
	if direction == hublive.StreamType_DOWNSTREAM {
...
func (s *StatsWorker) ParticipantID() hublive.ParticipantID {
...
func (s *StatsWorker) Flush(now time.Time, closeWait time.Duration) bool {
	...
	s.lock.Lock()
	stats := make([]*hublive.AnalyticsStat, 0, len(s.incomingPerTrack)+len(s.outgoingPerTrack))

	incomingPerTrack := s.incomingPerTrack
	s.incomingPerTrack = make(map[hublive.TrackID][]*hublive.AnalyticsStat)

	outgoingPerTrack := s.outgoingPerTrack
	s.outgoingPerTrack = make(map[hublive.TrackID][]*hublive.AnalyticsStat)

	closed := !s.closedAt.IsZero() && now.Sub(s.closedAt) > closeWait
	s.lock.Unlock()

	stats = s.collectStats(ts, hublive.StreamType_UPSTREAM, incomingPerTrack, stats)
	stats = s.collectStats(ts, hublive.StreamType_DOWNSTREAM, outgoingPerTrack, stats)
	if len(stats) > 0 {
...
func (s *StatsWorker) collectStats(
	...
	streamType hublive.StreamType,
	perTrack map[hublive.TrackID][]*hublive.AnalyticsStat,
	stats []*hublive.AnalyticsStat,
) []*hublive.AnalyticsStat {
...
func coalesce(stats []*hublive.AnalyticsStat) *hublive.AnalyticsStat {
	if len(stats) == 0 {
		return nil
	}
	...
	maxRtt := uint32(0)
	maxJitter := uint32(0)
	coalescedVideoLayers := make(map[int32]*hublive.AnalyticsVideoLayer)
	coalescedStream := &hublive.AnalyticsStream{}
	...
	for _, coalescedVideoLayer := range coalescedVideoLayers {
		if maxVideoLayer == -1 || maxVideoLayer < coalescedVideoLayer.Layer {
			...
			coalescedStream.VideoLayers = []*hublive.AnalyticsVideoLayer{coalescedVideoLayer}
		}
	}
	...
	stat := &hublive.AnalyticsStat{
		...
		Streams:     []*hublive.AnalyticsStream{coalescedStream},
...
func CondenseStat(stat *hublive.AnalyticsStat) (ps CondensedStat, ok bool) {
...
func isValid(stat *hublive.AnalyticsStat) bool {
	for _, analyticsStream := range stat.Streams {
