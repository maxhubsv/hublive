...
func (t *telemetryService) TrackStats(roomID hublive.RoomID, _roomName hublive.RoomName, key StatsKey, stat *hublive.AnalyticsStat) {
	t.enqueue(func() {
		direction := prometheus.Incoming
		if key.streamType == hublive.StreamType_DOWNSTREAM {
