...
func (r *RTPStatsSender) MaybeAdjustFirstPacketTime(publisherSRData *hublive.RTCPSenderReportState, tsOffset uint64) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RTPStatsSender) GetRtcpSenderReport(ssrc uint32, publisherSRData *hublive.RTCPSenderReportState, tsOffset uint64, passThrough bool) *rtcp.SenderReport {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RTPStatsSender) ToProto() *hublive.RTPStats {
	r.lock.RLock()
	defer r.lock.RUnlock()
