...
import (
	...
	"__GITHUB_HUBLIVE__mediatransportutil/pkg/latency"
	"__GITHUB_HUBLIVE__mediatransportutil/pkg/utils"
...
func (r *RTPStatsReceiver) getExtendedSenderReport(srData *hublive.RTCPSenderReportState) *hublive.RTCPSenderReportState {
	tsCycles := uint64(0)
	if r.srNewest != nil {
...
func (r *RTPStatsReceiver) checkOutOfOrderSenderReport(srData *hublive.RTCPSenderReportState) bool {
...
func (r *RTPStatsReceiver) checkRTPClockSkewForSenderReport(srData *hublive.RTCPSenderReportState) {
	if r.srNewest == nil {
		return
	}
...
func (r *RTPStatsReceiver) checkRTPClockSkewAgainstMediaPathForSenderReport(srData *hublive.RTCPSenderReportState) {
	if r.highestTime == 0 {
		return
	}
...
func (r *RTPStatsReceiver) updatePropagationDelayAndRecordSenderReport(srData *hublive.RTCPSenderReportState) {
	srTime := mediatransportutil.NtpTime(srData.NtpTimestamp).Time()
...
func (r *RTPStatsReceiver) SetRtcpSenderReportData(srData *hublive.RTCPSenderReportState) bool {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *RTPStatsReceiver) GetRtcpSenderReportData() *hublive.RTCPSenderReportState {
	r.lock.RLock()
...
func (r *RTPStatsReceiver) ToProto() *hublive.RTPStats {
	if r == nil {
		return nil
	}
