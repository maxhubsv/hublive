...
type DownTrackSpreader[T sender] struct {
	...
	downTracks       map[hublive.ParticipantID]T
	downTracksShadow []T
}
...
func NewDownTrackSpreader[T sender](params DownTrackSpreaderParams) *DownTrackSpreader[T] {
	d := &DownTrackSpreader[T]{
		params:     params,
		downTracks: make(map[hublive.ParticipantID]T),
	}
...
func (d *DownTrackSpreader[T]) ResetAndGetDownTracks() []T {
	...
	d.downTracks = make(map[hublive.ParticipantID]T)
...
func (d *DownTrackSpreader[T]) Free(subscriberID hublive.ParticipantID) {
	d.downTrackMu.Lock()
	defer d.downTrackMu.Unlock()
...
func (d *DownTrackSpreader[T]) HasDownTrack(subscriberID hublive.ParticipantID) bool {
	d.downTrackMu.RLock()
	defer d.downTrackMu.RUnlock()
