...
import (
	...
	"__GITHUB_HUBLIVE__mediatransportutil"
	...
	"github.com/maxhubsv/hublive-server/pkg/sfu/codecmunger"
	dd "github.com/maxhubsv/hublive-server/pkg/sfu/rtpextension/dependencydescriptor"
	"github.com/maxhubsv/hublive-server/pkg/sfu/rtpstats"
	sfuutils "github.com/maxhubsv/hublive-server/pkg/sfu/utils"
	"github.com/maxhubsv/hublive-server/pkg/sfu/videolayerselector"
	"github.com/maxhubsv/hublive-server/pkg/sfu/videolayerselector/temporallayerselector"
)
...
type refInfo struct {
	senderReport    *hublive.RTCPSenderReportState
	tsOffset        uint64
...
type Forwarder struct {
	...
	refVideoLayerMode        hublive.VideoLayer_Mode
	isDDAvailable            bool
...
func (f *Forwarder) DetermineCodec(codec webrtc.RTPCodecCapability, extensions []webrtc.RTPHeaderExtensionParameter, videoLayerMode hublive.VideoLayer_Mode) {
	f.lock.Lock()
	defer f.lock.Unlock()

	if videoLayerMode == hublive.VideoLayer_ONE_SPATIAL_LAYER_PER_STREAM_INCOMPLETE_RTCP_SR {
...
func (f *Forwarder) GetState() *hublive.RTPForwarderState {
	f.lock.RLock()
	...
	state := &hublive.RTPForwarderState{
		Started:                   f.started,
	...
	if vp8MungerState, ok := codecMungerState.(*hublive.VP8MungerState); ok {
		state.CodecMunger = &hublive.RTPForwarderState_Vp8Munger{
			Vp8Munger: vp8MungerState,
		}
	}

	state.SenderReportState = make([]*hublive.RTCPSenderReportState, len(f.refInfos))
...
func (f *Forwarder) SeedState(state *hublive.RTPForwarderState) {
...
func (f *Forwarder) getRefLayer() (int32, int32) {
	...
	if f.refVideoLayerMode == hublive.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
...
func (f *Forwarder) SetRefSenderReport(layer int32, srData *hublive.RTCPSenderReportState) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *Forwarder) GetSenderReportParams() (int32, bool, uint64, *hublive.RTCPSenderReportState) {
	f.lock.RLock()
	defer f.lock.RUnlock()
	...
	return currentLayerSpatial, f.refVideoLayerMode == hublive.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM, f.refInfos[refLayer].tsOffset, f.refInfos[refLayer].senderReport
}
...
func (f *Forwarder) Restart() {
	...
	f.refVideoLayerMode = hublive.VideoLayer_MODE_UNUSED
}
...
func (f *Forwarder) getRefLayerRTPTimestamp(ts uint32, refLayer, targetLayer int32) (uint32, error) {
	...
	if refLayer == targetLayer || f.refVideoLayerMode == hublive.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
		return ts, nil
	}
