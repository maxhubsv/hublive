...
import (
	...
	"github.com/maxhubsv/hublive-server/pkg/sfu/ccutils"
	"github.com/maxhubsv/hublive-server/pkg/sfu/connectionquality"
	"github.com/maxhubsv/hublive-server/pkg/sfu/pacer"
	"github.com/maxhubsv/hublive-server/pkg/sfu/packettrailer"
	act "github.com/maxhubsv/hublive-server/pkg/sfu/rtpextension/abscapturetime"
	...
	"github.com/maxhubsv/hublive-server/pkg/sfu/utils"
)
...
type DownTrackState struct {
	...
	ForwarderState                *hublive.RTPForwarderState
...
type DownTrackListener interface {
	OnBindAndConnected()
	OnStatsUpdate(stat *hublive.AnalyticsStat)
...
type DownTrackParams struct {
	...
	SubID                          hublive.ParticipantID
	StreamID                       string
	MaxTrack                       int
	PlayoutDelayLimit              *hublive.PlayoutDelay
	Pacer                          pacer.Pacer
...
func NewDownTrack(params DownTrackParams) (*DownTrack, error) {
	...
	d.connectionStats.OnStatsUpdate(func(_cs *connectionquality.ConnectionStats, stat *hublive.AnalyticsStat) {
...
func (d *DownTrack) SubscriberID() hublive.ParticipantID {
	// add `createdAt` to ensure repeated subscriptions from same subscriber to same publisher does not collide
	return hublive.ParticipantID(fmt.Sprintf("%s:%d", d.params.SubID, d.createdAt))
}
		...
		case *rtcp.ReceiverReport:
			...
			for _, r := range p.Reports {
				...
				if d.playoutDelay != nil {
					...
					if d.params.Source != hublive.TrackSource_SCREEN_SHARE {
...
func (d *DownTrack) GetConnectionScoreAndQuality() (float32, hublive.ConnectionQuality) {
	return d.connectionStats.GetScoreAndQuality()
}

func (d *DownTrack) GetTrackStats() *hublive.RTPStats {
...
func (d *DownTrack) handleRTCPSenderReportData(publisherSRData *hublive.RTCPSenderReportState, tsOffset uint64) {
