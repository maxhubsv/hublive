...
var (
	qualityTransitionScore = map[hublive.ConnectionQuality]float64{
		hublive.ConnectionQuality_GOOD: 80,
		hublive.ConnectionQuality_POOR: 40,
		hublive.ConnectionQuality_LOST: 20,
	}
)
...
func (q *qualityScorer) updateMuteAtLocked(isMuted bool, at time.Time) {
	if isMuted {
		...
		if q.score != qualityTransitionScore[hublive.ConnectionQuality_LOST] {
			q.score = cMaxScore
		}
	} else {
		q.unmutedAt = at
...
func (q *qualityScorer) updateAtLocked(stat *windowStat, at time.Time) {
	...
	if stat.packets+stat.packetsPadding == 0 {
		if !stat.lastRTCPAt.IsZero() && at.Sub(stat.lastRTCPAt) > stat.duration {
			reason = "rtcp"
			score = qualityTransitionScore[hublive.ConnectionQuality_LOST]
		} else {
			reason = "dry"
			score = qualityTransitionScore[hublive.ConnectionQuality_POOR]
		}
	} else {
...
func (q *qualityScorer) GetScoreAndQuality() (float32, hublive.ConnectionQuality) {
	q.lock.RLock()
...
func (q *qualityScorer) GetMOSAndQuality() (float32, hublive.ConnectionQuality) {
	q.lock.RLock()
	defer q.lock.RUnlock()
...
func scoreToConnectionQuality(score float64) hublive.ConnectionQuality {
	// R-factor -> HubLive.ConnectionQuality scale mapping roughly based on
	// https://www.itu.int/ITU-T/2005-2008/com12/emodelv1/tut.htm
	//
	// As there are only three levels in HubLive.ConnectionQuality scale,
	...
	if score > qualityTransitionScore[hublive.ConnectionQuality_GOOD] {
		return hublive.ConnectionQuality_EXCELLENT
	}

	if score > qualityTransitionScore[hublive.ConnectionQuality_POOR] {
		return hublive.ConnectionQuality_GOOD
	}

	if score > qualityTransitionScore[hublive.ConnectionQuality_LOST] {
		return hublive.ConnectionQuality_POOR
	}

	return hublive.ConnectionQuality_LOST
}
