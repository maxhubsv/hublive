...
type ConnectionStats struct {
	...
	onStatsUpdate func(cs *ConnectionStats, stat *hublive.AnalyticsStat)
...
func (cs *ConnectionStats) OnStatsUpdate(fn func(cs *ConnectionStats, stat *hublive.AnalyticsStat)) {
	cs.onStatsUpdate = fn
}
...
func (cs *ConnectionStats) GetScoreAndQuality() (float32, hublive.ConnectionQuality) {
...
func (cs *ConnectionStats) getStat() {
	...
	if cs.onStatsUpdate != nil && len(streams) != 0 {
		analyticsStreams := make([]*hublive.AnalyticsStream, 0, len(streams))
		...
		if len(analyticsStreams) != 0 {
			cs.onStatsUpdate(cs, &hublive.AnalyticsStat{
				Score:   score,
...
func toAnalyticsStream(
	...
	return &hublive.AnalyticsStream{
...
func toAnalyticsVideoLayer(layer int32, layerStats *rtpstats.RTPDeltaInfo) *hublive.AnalyticsVideoLayer {
	if layerStats == nil {
		return nil
	}

	avl := &hublive.AnalyticsVideoLayer{
		Layer:   layer,
