...
func LayerPresenceFromTrackInfo(mimeType mime.MimeType, trackInfo *hublive.TrackInfo) *[hublive.VideoQuality_HIGH + 1]bool {
	if trackInfo == nil {
		return nil
	}
	...
	var layerPresence [hublive.VideoQuality_HIGH + 1]bool
	for _, layer := range layers {
		// WARNING: comparing protobuf enum
		if layer.Quality <= hublive.VideoQuality_HIGH {
...
func RidToSpatialLayer(mimeType mime.MimeType, rid string, trackInfo *hublive.TrackInfo, ridSpace VideoLayersRid) int32 {
	...
	case ridSpace[0]:
		switch {
		case lp[hublive.VideoQuality_LOW] && lp[hublive.VideoQuality_MEDIUM] && lp[hublive.VideoQuality_HIGH]:
			fallthrough
		case lp[hublive.VideoQuality_LOW] && lp[hublive.VideoQuality_MEDIUM]:
			fallthrough
		case lp[hublive.VideoQuality_LOW] && lp[hublive.VideoQuality_HIGH]:
			fallthrough
		case lp[hublive.VideoQuality_MEDIUM] && lp[hublive.VideoQuality_HIGH]:
			return 0
...
func SpatialLayerToRid(mimeType mime.MimeType, layer int32, trackInfo *hublive.TrackInfo, ridSpace VideoLayersRid) string {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch layer {
		case 0:
...
func VideoQualityToRid(mimeType mime.MimeType, quality hublive.VideoQuality, trackInfo *hublive.TrackInfo, ridSpace VideoLayersRid) string {
	return SpatialLayerToRid(mimeType, VideoQualityToSpatialLayer(mimeType, quality, trackInfo), trackInfo, ridSpace)
}

func SpatialLayerToVideoQuality(mimeType mime.MimeType, layer int32, trackInfo *hublive.TrackInfo) hublive.VideoQuality {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch layer {
		case 0:
			return hublive.VideoQuality_LOW
		case 1:
			return hublive.VideoQuality_MEDIUM
		case 2:
			return hublive.VideoQuality_HIGH
		default:
			return hublive.VideoQuality_OFF
		}
	}
	...
	case 0:
		...
		case lp[hublive.VideoQuality_LOW] && lp[hublive.VideoQuality_HIGH]:
			fallthrough
		case lp[hublive.VideoQuality_LOW]:
			return hublive.VideoQuality_LOW

		case lp[hublive.VideoQuality_MEDIUM] && lp[hublive.VideoQuality_HIGH]:
			fallthrough
		case lp[hublive.VideoQuality_MEDIUM]:
			return hublive.VideoQuality_MEDIUM
...
func VideoQualityToSpatialLayer(mimeType mime.MimeType, quality hublive.VideoQuality, trackInfo *hublive.TrackInfo) int32 {
	lp := LayerPresenceFromTrackInfo(mimeType, trackInfo)
	if lp == nil {
		switch quality {
		case hublive.VideoQuality_LOW:
			return 0
		case hublive.VideoQuality_MEDIUM:
			return 1
		case hublive.VideoQuality_HIGH:
			return 2
		default:
...
func GetVideoLayerModeForMimeType(mimeType mime.MimeType, ti *hublive.TrackInfo) hublive.VideoLayer_Mode {
	if ti != nil {
	...
	return hublive.VideoLayer_MODE_UNUSED
}

func GetVideoLayersForMimeType(mimeType mime.MimeType, ti *hublive.TrackInfo) []*hublive.VideoLayer {
	var layers []*hublive.VideoLayer
	if ti != nil {
		for _, codec := range ti.Codecs {
			if mime.NormalizeMimeType(codec.MimeType) == mimeType {
				layers = codec.Layers
...
func GetSpatialLayerForRid(mimeType mime.MimeType, rid string, ti *hublive.TrackInfo) int32 {
	if ti == nil {
		return InvalidLayerSpatial
	}
...
func GetSpatialLayerForVideoQuality(mimeType mime.MimeType, quality hublive.VideoQuality, ti *hublive.TrackInfo) int32 {
	if ti == nil || quality == hublive.VideoQuality_OFF {
		return InvalidLayerSpatial
	}
...
func GetVideoQualityForSpatialLayer(mimeType mime.MimeType, spatialLayer int32, ti *hublive.TrackInfo) hublive.VideoQuality {
