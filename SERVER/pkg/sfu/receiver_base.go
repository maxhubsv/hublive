...
import (
	...
	"__GITHUB_HUBLIVE__mediatransportutil/pkg/bucket"
	...
	"github.com/maxhubsv/hublive-server/pkg/sfu/streamtracker"
...
type TrackReceiver interface {
	...
	Mime() mime.MimeType
	VideoLayerMode() hublive.VideoLayer_Mode
	...
	DeleteDownTrack(participantID hublive.ParticipantID)
...
type REDTransformer interface {
	...
	ForwardRTCPSenderReport(
		...
		layer int32,
		publisherSRData *hublive.RTCPSenderReportState,
	)
	GetDownTracks() []TrackSender
	ResyncDownTracks()
	OnStreamRestart()
...
type ReceiverBaseParams struct {
	...
	IsSelfClosing                bool
	OnNewBufferNeeded            func(int32, *hublive.TrackInfo) (buffer.BufferProvider, error)
	OnClosed                     func()
}
...
func NewReceiverBase(params ReceiverBaseParams, trackInfo *hublive.TrackInfo, codecState ReceiverCodecState) *ReceiverBase {
	r := &ReceiverBase{
		params:         params,
...
func (r *ReceiverBase) TrackInfo() *hublive.TrackInfo {
	r.bufferMu.RLock()
...
func (r *ReceiverBase) UpdateTrackInfo(ti *hublive.TrackInfo) {
	r.bufferMu.Lock()
...
func (r *ReceiverBase) TrackID() hublive.TrackID {
...
func (r *ReceiverBase) VideoLayerMode() hublive.VideoLayer_Mode {
...
func (r *ReceiverBase) DeleteDownTrack(subscriberID hublive.ParticipantID) {
...
func (r *ReceiverBase) getBufferLocked(layer int32) (buffer.BufferProvider, int32) {
	...
	if r.videoLayerMode == hublive.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM {
		layer = 0
	}
...
func (r *ReceiverBase) GetTrackStats() *hublive.RTPStats {
	r.bufferMu.RLock()
	defer r.bufferMu.RUnlock()

	allStats := make([]*hublive.RTPStats, 0, len(r.buffers))
	for _, buff := range r.buffers {
		if buff == nil {
			continue
		}
	...
	for r.forwardersGeneration.Load() == forwarderGeneration {
		...
		if r.Kind() == webrtc.RTPCodecTypeVideo {
			if spatialTrackers[spatialLayer] == nil {
				...
				if spatialTrackers[spatialLayer] == nil {
					if r.videoLayerMode == hublive.VideoLayer_MULTIPLE_SPATIAL_LAYERS_PER_STREAM && extPkt.DependencyDescriptor != nil {
