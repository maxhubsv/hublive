...
type MetricsCollectorProvider interface {
	...
	MetricsCollectorBatchReady(mb *hublive.MetricsBatch)
}
...
type MetricsCollector struct {
	...
	publisherRTTMetricId  map[hublive.ParticipantIdentity]int
	subscriberRTTMetricId int
	relayRTTMetricId      map[hublive.ParticipantIdentity]int
...
func (mc *MetricsCollector) AddPublisherRTT(participantIdentity hublive.ParticipantIdentity, rtt float32) {
	mc.lock.Lock()
	...
	if !ok {
		var err error
		metricId, err = mc.createTimeSeriesMetric(hublive.MetricLabel_PUBLISHER_RTT, participantIdentity)
		if err != nil {
...
func (mc *MetricsCollector) AddSubscriberRTT(rtt float32) {
	...
	if mc.subscriberRTTMetricId == utils.MetricsBatchBuilderInvalidTimeSeriesMetricId {
		var err error
		mc.subscriberRTTMetricId, err = mc.createTimeSeriesMetric(hublive.MetricLabel_SUBSCRIBER_RTT, mc.params.ParticipantIdentity)
		if err != nil {
			mc.params.Logger.Warnw("could not add time series metric for publisher RTT", err)
			return
		}
...
func (mc *MetricsCollector) AddRelayRTT(participantIdentity hublive.ParticipantIdentity, rtt float32) {
	mc.lock.Lock()
	defer mc.lock.Unlock()
	...
	if !ok {
		var err error
		metricId, err = mc.createTimeSeriesMetric(hublive.MetricLabel_SERVER_MESH_RTT, participantIdentity)
		if err != nil {
...
func (mc *MetricsCollector) getMetricsBatchAndReset() *hublive.MetricsBatch {
	mc.lock.Lock()
	mbb := mc.mbb
...
func (mc *MetricsCollector) reset() {
	...
	mc.publisherRTTMetricId = make(map[hublive.ParticipantIdentity]int)
	mc.subscriberRTTMetricId = utils.MetricsBatchBuilderInvalidTimeSeriesMetricId
	mc.relayRTTMetricId = make(map[hublive.ParticipantIdentity]int)
}

func (mc *MetricsCollector) createTimeSeriesMetric(
	label hublive.MetricLabel,
	participantIdentity hublive.ParticipantIdentity,
) (int, error) {
