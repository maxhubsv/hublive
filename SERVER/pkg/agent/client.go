...
import (
	...
	serverutils "github.com/maxhubsv/hublive-server/pkg/utils"
...
var jobTypeTopics = map[hublive.JobType]string{
	hublive.JobType_JT_ROOM:        RoomAgentTopic,
	hublive.JobType_JT_PUBLISHER:   PublisherAgentTopic,
	hublive.JobType_JT_PARTICIPANT: ParticipantAgentTopic,
}

type Client interface {
	// LaunchJob starts a room or participant job on an agent.
	// it will launch a job once for each worker in each namespace
	LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*hublive.Job]
	TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*hublive.JobState, error)
	Stop() error
}
...
func (c *agentClient) LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*hublive.Job] {
	var wg sync.WaitGroup
	ret := serverutils.NewIncrementalDispatcher[*hublive.Job]()
	defer func() {
...
func (c *agentClient) TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*hublive.JobState, error) {
...
func (c *agentClient) getDispatcher(agName string, jobType hublive.JobType) *serverutils.IncrementalDispatcher[string] {
	c.mu.Lock()
