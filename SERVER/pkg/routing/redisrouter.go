...
func (r *RedisRouter) GetNodeForRoom(_ context.Context, roomName hublive.RoomName) (*hublive.Node, error) {
	...
	return r.GetNode(hublive.NodeID(nodeID))
}

func (r *RedisRouter) SetNodeForRoom(_ context.Context, roomName hublive.RoomName, nodeID hublive.NodeID) error {
	return r.rc.HSet(r.ctx, NodeRoomKey, string(roomName), string(nodeID)).Err()
}

func (r *RedisRouter) ClearRoomState(_ context.Context, roomName hublive.RoomName) error {
...
func (r *RedisRouter) GetNode(nodeID hublive.NodeID) (*hublive.Node, error) {
...
func (r *RedisRouter) CreateRoom(ctx context.Context, req *hublive.CreateRoomRequest) (res *hublive.Room, err error) {
	rtcNode, err := r.GetNodeForRoom(ctx, hublive.RoomName(req.Name))
	if err != nil {
		return
	}

	return r.CreateRoomWithNodeID(ctx, req, hublive.NodeID(rtcNode.Id))
}
...
func (r *RedisRouter) StartParticipantSignal(ctx context.Context, roomName hublive.RoomName, pi ParticipantInit) (res StartParticipantSignalResults, err error) {
	rtcNode, err := r.GetNodeForRoom(ctx, roomName)
	if err != nil {
		return
	}

	return r.StartParticipantSignalWithNodeID(ctx, roomName, pi, hublive.NodeID(rtcNode.Id))
}
