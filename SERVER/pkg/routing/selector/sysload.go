...
func (s *SystemLoadSelector) filterNodes(nodes []*hublive.Node) ([]*hublive.Node, error) {
	nodes = GetAvailableNodes(nodes)
	if len(nodes) == 0 {
		return nil, ErrNoAvailableNodes
	}

	nodesLowLoad := make([]*hublive.Node, 0)
...
func (s *SystemLoadSelector) SelectNode(nodes []*hublive.Node) (*hublive.Node, error) {
