...
func createTestNode(id string, cpuLoad float32, numRooms int32, numClients int32, state hublive.NodeState) *hublive.Node {
	return &hublive.Node{
		Id:    id,
		State: state,
...
func TestAnySelector_SelectNode_TwoChoice(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			algorithm: "twochoice",
			nodes: []*hublive.Node{
				createTestNode("node1", 0.8, 5, 10, hublive.NodeState_SERVING),
				createTestNode("node2", 0.3, 2, 5, hublive.NodeState_SERVING),
				createTestNode("node3", 0.6, 3, 8, hublive.NodeState_SERVING),
				createTestNode("node4", 0.9, 6, 12, hublive.NodeState_SERVING),
			},
			wantErr:     "",
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*hublive.Node{
				createTestNode("node1", 0.5, 8, 15, hublive.NodeState_SERVING),
				createTestNode("node2", 0.4, 2, 5, hublive.NodeState_SERVING),
				createTestNode("node3", 0.6, 12, 20, hublive.NodeState_SERVING),
			},
			wantErr:     "",
			expected:    "",      // Not determinstic selection, so no specific expected node
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*hublive.Node{
				createTestNode("node1", 0.5, 3, 25, hublive.NodeState_SERVING),
				createTestNode("node2", 0.4, 2, 5, hublive.NodeState_SERVING),
				createTestNode("node3", 0.6, 4, 30, hublive.NodeState_SERVING),
			},
			wantErr:     "",
			expected:    "",      // Not determinstic selection, so no specific expected node
		...
		},
		{
			...
			nodes: []*hublive.Node{
				{
					Id:    "node1",
					State: hublive.NodeState_SHUTTING_DOWN,
					Stats: &hublive.NodeStats{
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*hublive.Node{
				createTestNode("node1", 0.5, 3, 10, hublive.NodeState_SERVING),
			},
			wantErr:     "",
		...
		},
		{
			...
			algorithm: "twochoice",
			nodes: []*hublive.Node{
				createTestNode("node1", 0.8, 5, 15, hublive.NodeState_SERVING),
				createTestNode("node2", 0.3, 2, 5, hublive.NodeState_SERVING),
			},
			wantErr:     "",
...
func TestAnySelector_SelectNode_TwoChoice_Probabilistic_Behavior(t *testing.T) {
	...
	nodes := []*hublive.Node{
		createTestNode("node1", 0.95, 10, 20, hublive.NodeState_SERVING), // Very high load
		createTestNode("node2", 0.1, 1, 2, hublive.NodeState_SERVING),    // Low load
		createTestNode("node3", 0.5, 9, 18, hublive.NodeState_SERVING),   // Medium load
		createTestNode("node4", 0.85, 8, 16, hublive.NodeState_SERVING),  // High load
	}
