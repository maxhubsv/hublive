...
type SignalClient interface {
	ActiveCount() int
	StartParticipantSignal(ctx context.Context, roomName hublive.RoomName, pi ParticipantInit, nodeID hublive.NodeID) (connectionID hublive.ConnectionID, reqSink MessageSink, resSource MessageSource, err error)
}
...
func NewSignalClient(nodeID hublive.NodeID, bus psrpc.MessageBus, config config.SignalRelayConfig) (SignalClient, error) {
...
func (r *signalClient) StartParticipantSignal(
	...
	connectionID = hublive.ConnectionID(guid.New("CO_"))
...
func (e signalRequestMessageWriter) Write(seq uint64, close bool, msgs []proto.Message) *rpc.RelaySignalRequest {
	r := &rpc.RelaySignalRequest{
		Seq:      seq,
		Requests: make([]*hublive.SignalRequest, 0, len(msgs)),
		Close:    close,
	}
	for _, m := range msgs {
		r.Requests = append(r.Requests, m.(*hublive.SignalRequest))
	}
	return r
}
...
func (s *signalMessageSink[SendType, RecvType]) ConnectionID() hublive.ConnectionID {
