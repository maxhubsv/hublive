...
func handleICEExchange(t *testing.T, a, b *PCTransport, ah, bh *transportfakes.FakeHandler) {
	ah.OnICECandidateCalls(func(candidate *webrtc.ICECandidate, target hublive.SignalTarget) error {
		if candidate == nil {
			return nil
		}
	...
	bh.OnICECandidateCalls(func(candidate *webrtc.ICECandidate, target hublive.SignalTarget) error {
		if candidate == nil {
			return nil
		}
...
func TestConfigureAudioTransceiver(t *testing.T) {
	for _, testcase := range []struct {
		...
		t.Run(fmt.Sprintf("nack=%v,stereo=%v", testcase.nack, testcase.stereo), func(t *testing.T) {
			...
			registerCodecs(&me, []*hublive.Codec{{Mime: mime.MimeTypeOpus.String()}}, RTCPFeedbackConfig{Audio: []webrtc.RTCPFeedback{{Type: webrtc.TypeRTCPFBNACK}}}, false)
