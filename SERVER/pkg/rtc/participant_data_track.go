...
func (p *ParticipantImpl) HandlePublishDataTrackRequest(req *hublive.PublishDataTrackRequest) {
	if !p.CanPublishData() || !p.params.EnableDataTracks {
		...
		p.sendRequestResponse(&hublive.RequestResponse{
			...
			Request: &hublive.RequestResponse_PublishDataTrack{
	...
	if req.PubHandle == 0 || req.PubHandle > 65535 {
		...
		p.sendRequestResponse(&hublive.RequestResponse{
			Reason:  hublive.RequestResponse_INVALID_HANDLE,
	...
	if len(req.Name) == 0 || len(req.Name) > 256 {
		...
		p.sendRequestResponse(&hublive.RequestResponse{
			Reason:  hublive.RequestResponse_INVALID_NAME,
...
func (p *ParticipantImpl) HandleUnpublishDataTrackRequest(req *hublive.UnpublishDataTrackRequest) {
	...
	if dt == nil {
		...
		p.sendRequestResponse(&hublive.RequestResponse{
			...
			Request: &hublive.RequestResponse_UnpublishDataTrack{
...
func (p *ParticipantImpl) HandleUpdateDataSubscription(req *hublive.UpdateDataSubscription) {
