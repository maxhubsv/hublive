...
type Room struct {
	...
	protoRoom  *hublive.Room
	internal   *hublive.RoomInternal
	protoProxy *utils.ProtoProxy[*hublive.Room]
	...
	participants              map[hublive.ParticipantIdentity]types.LocalParticipant
	participantOpts           map[hublive.ParticipantIdentity]*ParticipantOptions
	participantRequestSources map[hublive.ParticipantIdentity]routing.MessageSource
	hasPublished              map[hublive.ParticipantIdentity]bool
	agentParticpants          map[hublive.ParticipantIdentity]*agentJob
	bufferFactory             *buffer.FactoryOfBufferFactory

	// batch update participant info for non-publishers
	batchedUpdates   map[hublive.ParticipantIdentity]*ParticipantUpdate
	...
	disconnectSignalOnResumeParticipants           map[hublive.ParticipantIdentity]time.Time
	disconnectSignalOnResumeNoMessagesParticipants map[hublive.ParticipantIdentity]*disconnectSignalOnResumeNoMessages
...
func newAgentDispatch(ad *hublive.AgentDispatch) *agentDispatch {
...
func newAgentJob(j *hublive.Job) *agentJob {
	return &agentJob{
		Job:  j,
...
func NewRoom(
	...
	r := &Room{
		...
		participants:                         make(map[hublive.ParticipantIdentity]types.LocalParticipant),
		participantOpts:                      make(map[hublive.ParticipantIdentity]*ParticipantOptions),
		participantRequestSources:            make(map[hublive.ParticipantIdentity]routing.MessageSource),
		hasPublished:                         make(map[hublive.ParticipantIdentity]bool),
		agentParticpants:                     make(map[hublive.ParticipantIdentity]*agentJob),
		...
		batchedUpdates:                       make(map[hublive.ParticipantIdentity]*ParticipantUpdate),
		closed:                               make(chan struct{}),
		trailer:                              []byte(utils.RandomSecret()),
		disconnectSignalOnResumeParticipants: make(map[hublive.ParticipantIdentity]time.Time),
		disconnectSignalOnResumeNoMessagesParticipants: make(map[hublive.ParticipantIdentity]*disconnectSignalOnResumeNoMessages),
...
func (r *Room) ToProto() *hublive.Room {
	return r.protoProxy.Get()
}

func (r *Room) Name() hublive.RoomName {
	return hublive.RoomName(r.protoRoom.Name)
}

func (r *Room) ID() hublive.RoomID {
	return hublive.RoomID(r.protoRoom.Sid)
}
...
func (r *Room) GetParticipant(identity hublive.ParticipantIdentity) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) GetParticipantByID(participantID hublive.ParticipantID) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) GetActiveSpeakers() []*hublive.SpeakerInfo {
	participants := r.GetParticipants()
	speakers := make([]*hublive.SpeakerInfo, 0, len(participants))
	for _, p := range participants {
		level, active := p.GetAudioLevel()
		if !active {
			continue
		}
		speakers = append(speakers, &hublive.SpeakerInfo{
			Sid:    string(p.ID()),
			Level:  float32(level),
			Active: active,
		})
...
func (r *Room) Internal() *hublive.RoomInternal {
	return r.internal
}
...
func (r *Room) Join(
	...
	r.launchTargetAgents(maps.Values(r.agentDispatches), participant, hublive.JobType_JT_PARTICIPANT)
...
func (r *Room) ReplaceParticipantRequestSource(identity hublive.ParticipantIdentity, reqSource routing.MessageSource) {
	r.lock.Lock()
...
func (r *Room) GetParticipantRequestSource(identity hublive.ParticipantIdentity) routing.MessageSource {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) ResumeParticipant(
	...
	if err := p.HandleReconnectAndSendResponse(reason, &hublive.ReconnectResponse{
...
func (r *Room) HandleSyncState(participant types.LocalParticipant, state *hublive.SyncState) error {
	if state != nil {
...
func (r *Room) onSyncState(participant types.LocalParticipant, state *hublive.SyncState) error {
	...
	for _, pubTrack := range pubTracks {
		...
		for _, existingPubTrack := range existingPubTracks {
			if existingPubTrack.ID() == hublive.TrackID(ti.Sid) {
				found = true
				break
			}
		}
		if !found {
			...
			found = participant.GetPendingTrack(hublive.TrackID(ti.Sid)) != nil
		}
		if !found {
	...
	for _, pubDataTrack := range pubDataTracks {
		...
		for _, existingPubDataTrack := range existingPubDataTracks {
			if existingPubDataTrack.ID() == hublive.TrackID(dti.Sid) {
				found = true
				break
			}
		}
	...
	for _, trackSid := range state.TrackSidsDisabled {
		participant.UpdateSubscribedTrackSettings(hublive.TrackID(trackSid), &hublive.UpdateTrackSettings{Disabled: true})
	}

	participant.HandleUpdateSubscriptions(
		hublive.StringsAsIDs[hublive.TrackID](state.Subscription.TrackSids),
...
func (r *Room) onUpdateSubscriptionPermission(participant types.LocalParticipant, subscriptionPermission *hublive.SubscriptionPermission) error {
...
func (r *Room) ResolveMediaTrackForSubscriber(sub types.LocalParticipant, trackID hublive.TrackID) types.MediaResolverResult {
...
func (r *Room) ResolveDataTrackForSubscriber(sub types.LocalParticipant, trackID hublive.TrackID) types.DataResolverResult {
...
func (r *Room) SendDataPacket(dp *hublive.DataPacket, kind hublive.DataPacket_Kind) {
...
func (r *Room) GetAgentDispatches(dispatchID string) ([]*hublive.AgentDispatch, error) {
	r.lock.RLock()
	defer r.lock.RUnlock()

	var ret []*hublive.AgentDispatch
...
func (r *Room) AddAgentDispatch(dispatch *hublive.AgentDispatch) (*hublive.AgentDispatch, error) {
	...
	for _, p := range r.participants {
		if p.IsPublisher() {
			r.launchTargetAgents([]*agentDispatch{ad}, p, hublive.JobType_JT_PUBLISHER)
		}
		r.launchTargetAgents([]*agentDispatch{ad}, p, hublive.JobType_JT_PARTICIPANT)
	}
	r.lock.RUnlock()
...
func (r *Room) DeleteAgentDispatch(dispatchID string) (*hublive.AgentDispatch, error) {
	r.lock.Lock()
	...
	go func() {
		...
		for _, j := range jobs {
			...
			if state.ParticipantIdentity != "" {
				r.lock.RLock()
				agentJob := r.agentParticpants[hublive.ParticipantIdentity(state.ParticipantIdentity)]
				p := r.participants[hublive.ParticipantIdentity(state.ParticipantIdentity)]
				r.lock.RUnlock()
...
func (r *Room) onSimulateScenario(participant types.LocalParticipant, simulateScenario *hublive.SimulateScenario) error {
	switch scenario := simulateScenario.Scenario.(type) {
	case *hublive.SimulateScenario_SpeakerUpdate:
		...
		go func() {
			...
			r.sendSpeakerChanges([]*hublive.SpeakerInfo{{
	...
	case *hublive.SimulateScenario_Migration:
	...
	case *hublive.SimulateScenario_NodeFailure:
	...
	case *hublive.SimulateScenario_ServerLeave:
	...
	case *hublive.SimulateScenario_SwitchCandidateProtocol:
		r.logger.Infow("simulating switch candidate protocol", "participant", participant.Identity())
		participant.ICERestart(&hublive.ICEConfig{
			PreferenceSubscriber: hublive.ICECandidateType(scenario.SwitchCandidateProtocol),
			PreferencePublisher:  hublive.ICECandidateType(scenario.SwitchCandidateProtocol),
		})
	case *hublive.SimulateScenario_SubscriberBandwidth:
	...
	case *hublive.SimulateScenario_DisconnectSignalOnResume:
		...
		r.simulationLock.Unlock()
	case *hublive.SimulateScenario_DisconnectSignalOnResumeNoMessages:
...
func (r *Room) createJoinResponseLocked(
	participant types.LocalParticipant,
	iceServers []*hublive.ICEServer,
) *hublive.JoinResponse {
	iceConfig := participant.GetICEConfig()
	hasICEFallback := iceConfig.GetPreferencePublisher() != hublive.ICECandidateType_ICT_NONE || iceConfig.GetPreferenceSubscriber() != hublive.ICECandidateType_ICT_NONE
	return &hublive.JoinResponse{
		Room:        r.ToProto(),
...
func (r *Room) onTrackPublished(participant types.Participant, track types.MediaTrack) {
	...
	for _, existingParticipant := range r.participants {
		...
		if existingParticipant.State() != hublive.ParticipantInfo_ACTIVE {
	...
	if !hasPublished {
		r.lock.RLock()
		r.launchTargetAgents(maps.Values(r.agentDispatches), participant, hublive.JobType_JT_PUBLISHER)
		r.lock.RUnlock()
	...
	if participant.Kind() != hublive.ParticipantInfo_EGRESS && r.internal != nil && r.internal.TrackEgress != nil {
		go func() {
...
func (r *Room) onStateChange(p types.LocalParticipant) {
	...
	switch p.State() {
	case hublive.ParticipantInfo_ACTIVE:
		...
		meta := &hublive.AnalyticsClientMeta{
	...
	case hublive.ParticipantInfo_DISCONNECTED:
		// remove participant from room
		go r.RemoveParticipant(p.Identity(), p.ID(), p.CloseReason())
	}
}

func (r *Room) onDataMessage(source types.LocalParticipant, kind hublive.DataPacket_Kind, dp *hublive.DataPacket) {
	if kind == hublive.DataPacket_RELIABLE && source != nil && dp.GetSequence() > 0 {
		...
		r.dataMessageCache.Add(&types.DataMessageCache{
			...
			Seq:            dp.GetSequence(),
			Data:           data,
			DestIdentities: hublive.StringsAsIDs[hublive.ParticipantIdentity](dp.DestinationIdentities),
		}, len(data))
	}
...
func (r *Room) onMetrics(source types.Participant, dp *hublive.DataPacket) {
	BroadcastMetricsForRoom(r, source, dp, r.logger)
}

func (r *Room) onSubscribeStatusChanged(participant types.LocalParticipant, publisherID hublive.ParticipantID, subscribed bool) {
	if subscribed {
		pub := r.GetParticipantByID(publisherID)
		if pub != nil && pub.State() == hublive.ParticipantInfo_ACTIVE {
			...
			if active {
				_ = participant.SendSpeakerUpdate([]*hublive.SpeakerInfo{
					{
						Sid:    string(pub.ID()),
						Level:  float32(level),
						Active: active,
			...
			if cq := pub.GetConnectionQuality(); cq != nil {
				update := &hublive.ConnectionQualityUpdate{}
...
func (r *Room) UpdateSubscriptions(
	...
	for _, pt := range participantTracks {
		for _, trackID := range hublive.StringsAsIDs[hublive.TrackID](pt.TrackSids) {
			if subscribe {
				participant.SubscribeToTrack(trackID, false)
			} else {
				participant.UnsubscribeFromTrack(trackID)
...
func (r *Room) onUpdateDataSubscriptions(participant types.LocalParticipant, req *hublive.UpdateDataSubscription) {
	for _, update := range req.Updates {
		trackID := hublive.TrackID(update.TrackSid)
		if update.Subscribe {
...
func (r *Room) broadcastParticipantState(p types.Participant, opts broadcastOptions) {
	...
	if !opts.skipSource {
		defer func() {
			...
			if lp, ok := p.(types.LocalParticipant); ok {
				err := lp.SendParticipantUpdate([]*hublive.ParticipantInfo{pi})
				if err != nil {
	...
	updates := PushAndDequeueUpdates(
		...
		p.CloseReason(),
		opts.immediate,
		r.GetParticipant(hublive.ParticipantIdentity(pi.Identity)),
		r.batchedUpdates,
	)
...
func (r *Room) sendSpeakerChanges(speakers []*hublive.SpeakerInfo) {
	for _, p := range r.GetParticipants() {
...
func (r *Room) updateProto() *hublive.Room {
	r.lock.RLock()
...
func (r *Room) changeUpdateWorker() {
	...
	for !r.IsClosed() {
		...
		case <-subTicker.C:
			...
			r.batchedUpdates = make(map[hublive.ParticipantIdentity]*ParticipantUpdate)
			r.batchedUpdatesMu.Unlock()
...
func (r *Room) audioUpdateWorker() {
	lastActiveMap := make(map[hublive.ParticipantID]*hublive.SpeakerInfo)
	for {
		if r.IsClosed() {
			return
		}
		...
		changedSpeakers := make([]*hublive.SpeakerInfo, 0, len(activeSpeakers))
		nextActiveMap := make(map[hublive.ParticipantID]*hublive.SpeakerInfo, len(activeSpeakers))
		for _, speaker := range activeSpeakers {
			prev := lastActiveMap[hublive.ParticipantID(speaker.Sid)]
			if prev == nil || prev.Level != speaker.Level {
				changedSpeakers = append(changedSpeakers, speaker)
			}
			nextActiveMap[hublive.ParticipantID(speaker.Sid)] = speaker
		}
...
func (r *Room) connectionQualityWorker() {
	...
	prevConnectionInfos := make(map[hublive.ParticipantID]*hublive.ConnectionQualityInfo)
	// send updates to only users that are subscribed to each other
	for !r.IsClosed() {
		<-ticker.C

		participants := r.GetParticipants()
		nowConnectionInfos := make(map[hublive.ParticipantID]*hublive.ConnectionQualityInfo, len(participants))
		...
		maybeAddToUpdate := func(pID hublive.ParticipantID, update *hublive.ConnectionQualityUpdate) {
		...
		for _, op := range participants {
			if !op.ProtocolVersion().SupportsConnectionQuality() || op.State() != hublive.ParticipantInfo_ACTIVE {
				continue
			}
			update := &hublive.ConnectionQualityUpdate{}
...
func (r *Room) launchTargetAgents(ads []*agentDispatch, p types.Participant, jobType hublive.JobType) {
...
func (r *Room) handleNewJobs(ad *hublive.AgentDispatch, inc *sutils.IncrementalDispatcher[*hublive.Job]) {
	inc.ForEach(func(job *hublive.Job) {
		...
		if job.State != nil && job.State.ParticipantIdentity != "" {
			r.agentParticpants[hublive.ParticipantIdentity(job.State.ParticipantIdentity)] = newAgentJob(job)
		}
		r.lock.Unlock()
	})
}
...
func (r *Room) createAgentDispatch(dispatch *hublive.AgentDispatch) (*agentDispatch, error) {
	dispatch.State = &hublive.AgentDispatchState{
...
func (r *Room) createAgentDispatchFromRoomDispatch(rad *hublive.RoomAgentDispatch) (*agentDispatch, error) {
	return r.createAgentDispatch(&hublive.AgentDispatch{
...
func (r *Room) createAgentDispatchesFromRoomAgent() {
	...
	if len(roomDisp) == 0 {
		...
		roomDisp = []*hublive.RoomAgentDispatch{{}}
	}
...
func (r *Room) IsDataMessageUserPacketDuplicate(up *hublive.UserPacket) bool {
	return r.userPacketDeduper.IsDuplicate(up)
}

func (r *Room) GetCachedReliableDataMessage(seqs map[hublive.ParticipantID]uint32) []*types.DataMessageCache {
...
func (l *localParticipantListener) OnMetrics(p types.Participant, dp *hublive.DataPacket) {
...
func (l *localParticipantListener) OnDataMessage(p types.LocalParticipant, kind hublive.DataPacket_Kind, dp *hublive.DataPacket) {
...
func (l *localParticipantListener) OnSubscribeStatusChanged(p types.LocalParticipant, publisherID hublive.ParticipantID, subscribed bool) {
...
func (l *localParticipantListener) OnUpdateSubscriptionPermission(p types.LocalParticipant, subscriptionPermission *hublive.SubscriptionPermission) error {
	return l.room.onUpdateSubscriptionPermission(p, subscriptionPermission)
}

func (l *localParticipantListener) OnUpdateDataSubscriptions(p types.LocalParticipant, req *hublive.UpdateDataSubscription) {
	l.room.onUpdateDataSubscriptions(p, req)
}

func (l *localParticipantListener) OnSyncState(p types.LocalParticipant, state *hublive.SyncState) error {
	return l.room.onSyncState(p, state)
}

func (l *localParticipantListener) OnSimulateScenario(p types.LocalParticipant, simulateScenario *hublive.SimulateScenario) error {
...
func (l participantTelemetryListener) OnTrackPublishRequested(pID hublive.ParticipantID, identity hublive.ParticipantIdentity, ti *hublive.TrackInfo) {
	l.room.telemetry.TrackPublishRequested(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti)
}

func (l participantTelemetryListener) OnTrackPublished(pID hublive.ParticipantID, identity hublive.ParticipantIdentity, ti *hublive.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackPublished(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackUnpublished(pID hublive.ParticipantID, identity hublive.ParticipantIdentity, ti *hublive.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackUnpublished(context.Background(), l.room.ID(), l.room.Name(), pID, identity, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackSubscribeRequested(pID hublive.ParticipantID, ti *hublive.TrackInfo) {
	l.room.telemetry.TrackSubscribeRequested(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackSubscribed(pID hublive.ParticipantID, ti *hublive.TrackInfo, publisherInfo *hublive.ParticipantInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackSubscribed(context.Background(), l.room.ID(), l.room.Name(), pID, ti, publisherInfo, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackUnsubscribed(pID hublive.ParticipantID, ti *hublive.TrackInfo, shouldSendEvent bool) {
	l.room.telemetry.TrackUnsubscribed(context.Background(), l.room.ID(), l.room.Name(), pID, ti, shouldSendEvent)
}

func (l participantTelemetryListener) OnTrackSubscribeFailed(pID hublive.ParticipantID, ti hublive.TrackID, err error, isUserError bool) {
	l.room.telemetry.TrackSubscribeFailed(context.Background(), l.room.ID(), l.room.Name(), pID, ti, err, isUserError)
}

func (l participantTelemetryListener) OnTrackMuted(pID hublive.ParticipantID, ti *hublive.TrackInfo) {
	l.room.telemetry.TrackMuted(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackUnmuted(pID hublive.ParticipantID, ti *hublive.TrackInfo) {
	l.room.telemetry.TrackUnmuted(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackPublishedUpdate(pID hublive.ParticipantID, ti *hublive.TrackInfo) {
	l.room.telemetry.TrackPublishedUpdate(context.Background(), l.room.ID(), l.room.Name(), pID, ti)
}

func (l participantTelemetryListener) OnTrackMaxSubscribedVideoQuality(pID hublive.ParticipantID, ti *hublive.TrackInfo, mime mime.MimeType, maxQuality hublive.VideoQuality) {
	l.room.telemetry.TrackMaxSubscribedVideoQuality(context.Background(), l.room.ID(), l.room.Name(), pID, ti, mime, maxQuality)
}

func (l participantTelemetryListener) OnTrackPublishRTPStats(pID hublive.ParticipantID, trackID hublive.TrackID, mimeType mime.MimeType, layer int, stats *hublive.RTPStats) {
	l.room.telemetry.TrackPublishRTPStats(context.Background(), l.room.ID(), l.room.Name(), pID, trackID, mimeType, layer, stats)
}

func (l participantTelemetryListener) OnTrackSubscribeRTPStats(pID hublive.ParticipantID, trackID hublive.TrackID, mimeType mime.MimeType, stats *hublive.RTPStats) {
	l.room.telemetry.TrackSubscribeRTPStats(context.Background(), l.room.ID(), l.room.Name(), pID, trackID, mimeType, stats)
}

func (l participantTelemetryListener) OnTrackStats(key telemetry.StatsKey, stat *hublive.AnalyticsStat) {
...
func BroadcastDataPacketForRoom(
	...
	utils.ParallelExec(destParticipants, dataForwardLoadBalanceThreshold, 1, func(op types.LocalParticipant) {
		op.SendDataMessage(kind, dpData, hublive.ParticipantID(dp.GetParticipantSid()), dp.GetSequence())
	})
}
...
func BroadcastMetricsForRoom(r types.Room, source types.Participant, dp *hublive.DataPacket, logger logger.Logger) {
...
func CompareParticipant(pi1 *hublive.ParticipantInfo, pi2 *hublive.ParticipantInfo) int {
...
type ParticipantUpdate struct {
	ParticipantInfo         *hublive.ParticipantInfo
...
func PushAndDequeueUpdates(
	...
	cache map[hublive.ParticipantIdentity]*ParticipantUpdate,
) []*ParticipantUpdate {
	var updates []*ParticipantUpdate
	identity := hublive.ParticipantIdentity(pi.Identity)
	...
	if existing != nil {
		if pi.Sid == existing.ParticipantInfo.Sid {
			...
			if CompareParticipant(existing.ParticipantInfo, pi) < 0 {
				...
				shouldSend = true
				existing.ParticipantInfo.State = hublive.ParticipantInfo_DISCONNECTED
...
func SendParticipantUpdates(updates []*ParticipantUpdate, participants []types.LocalParticipant, batchTargetSize int) {
	...
	var filteredUpdates []*hublive.ParticipantInfo
	...
	var fullUpdates []*hublive.ParticipantInfo
	for _, update := range updates {
...
) []*hublive.ParticipantInfo {
	var lpIdentity hublive.ParticipantIdentity
	if lp != nil {
		lpIdentity = lp.Identity()
	}

	pInfos := make([]*hublive.ParticipantInfo, 0, len(allParticipants))
