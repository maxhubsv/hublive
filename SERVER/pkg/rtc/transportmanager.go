...
type TransportManager struct {
	...
	pendingDataChannelsPublisher []*hublive.DataChannelInfo
	...
	onICEConfigChanged func(iceConfig *hublive.ICEConfig)
...
func NewTransportManager(params TransportManagerParams) (*TransportManager, error) {
	...
	lgr := LoggerWithPCTarget(params.Logger, hublive.SignalTarget_PUBLISHER)
	...
	if !t.params.UseOneShotSignallingMode && !t.params.UseSinglePeerConnection {
		lgr := LoggerWithPCTarget(params.Logger, hublive.SignalTarget_SUBSCRIBER)
...
func (t *TransportManager) SendDataMessage(kind hublive.DataPacket_Kind, data []byte) error {
...
func (t *TransportManager) SendDataMessageUnlabeled(data []byte, useRaw bool, sender hublive.ParticipantIdentity) error {
	// downstream data is sent via primary peer connection
...
func (t *TransportManager) createDataChannelsForSubscriber(pendingDataChannels []*hublive.DataChannelInfo) error {
	var (
...
func (t *TransportManager) AddICECandidate(candidate webrtc.ICECandidateInit, target hublive.SignalTarget) {
	switch target {
	case hublive.SignalTarget_PUBLISHER:
		if t.publisher != nil {
	...
	case hublive.SignalTarget_SUBSCRIBER:
		if t.subscriber != nil {
...
func (t *TransportManager) HandleClientReconnect(reason hublive.ReconnectReason) {
	var (
		isShort              bool
	...
	switch reason {
	case hublive.ReconnectReason_RR_PUBLISHER_FAILED:
		if t.publisher != nil {
			resetShortConnection = true
			isShort, duration = t.publisher.IsShortConnection(time.Now())
		}

	case hublive.ReconnectReason_RR_SUBSCRIBER_FAILED:
		if t.subscriber != nil {
			resetShortConnection = true
...
func (t *TransportManager) ICERestart(iceConfig *hublive.ICEConfig) error {
...
func (t *TransportManager) OnICEConfigChanged(f func(iceConfig *hublive.ICEConfig)) {
	t.lock.Lock()
	t.onICEConfigChanged = f
	t.lock.Unlock()
}

func (t *TransportManager) SetICEConfig(iceConfig *hublive.ICEConfig) {
	if iceConfig != nil {
		t.configureICE(iceConfig, true)
	}
}

func (t *TransportManager) GetICEConfig() *hublive.ICEConfig {
	t.lock.RLock()
...
func (t *TransportManager) configureICE(iceConfig *hublive.ICEConfig, reset bool) {
	t.lock.Lock()
	...
	if iceConfig.PreferenceSubscriber != hublive.ICECandidateType_ICT_NONE {
		t.mediaLossProxy.OnMediaLossUpdate(nil)
	}

	if t.publisher != nil {
		t.publisher.SetPreferTCP(iceConfig.PreferencePublisher == hublive.ICECandidateType_ICT_TCP)
	}
	if t.subscriber != nil {
		t.subscriber.SetPreferTCP(iceConfig.PreferenceSubscriber == hublive.ICECandidateType_ICT_TCP)
	}
...
func (t *TransportManager) handleConnectionFailed(isShortLived bool) {
	...
	getNext := func(ic *hublive.ICEConfig) hublive.ICECandidateType {
		...
		return hublive.ICECandidateType_ICT_NONE
	}

	var preferNext hublive.ICECandidateType
	if isShortLived {
	...
	t.configureICE(&hublive.ICEConfig{
...
func (t *TransportManager) SetMigrateInfo(
	...
	t.lock.Lock()
	t.pendingDataChannelsPublisher = make([]*hublive.DataChannelInfo, 0, len(dataChannels))
	pendingDataChannelsSubscriber := make([]*hublive.DataChannelInfo, 0, len(dataChannels))
	for _, dci := range dataChannels {
		if dci.Target == hublive.SignalTarget_SUBSCRIBER {
