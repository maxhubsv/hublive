...
type UpTrackManager struct {
	...
	publishedTracks        map[hublive.TrackID]types.MediaTrack
	subscriptionPermission *hublive.SubscriptionPermission
	// subscriber permission for published tracks
	subscriberPermissions map[hublive.ParticipantIdentity]*hublive.TrackPermission // subscriberIdentity => *hublive.TrackPermission
...
func NewUpTrackManager(params UpTrackManagerParams) *UpTrackManager {
	return &UpTrackManager{
		params:          params,
		publishedTracks: make(map[hublive.TrackID]types.MediaTrack),
	}
}
...
func (u *UpTrackManager) Close(isExpectedToResume bool) {
	...
	u.publishedTracks = make(map[hublive.TrackID]types.MediaTrack)
	u.lock.Unlock()
...
func (u *UpTrackManager) ToProto() []*hublive.TrackInfo {
	u.lock.RLock()
	defer u.lock.RUnlock()

	var trackInfos []*hublive.TrackInfo
...
func (u *UpTrackManager) SetPublishedTrackMuted(trackID hublive.TrackID, muted bool) (types.MediaTrack, bool) {
	changed := false
...
func (u *UpTrackManager) GetPublishedTrack(trackID hublive.TrackID) types.MediaTrack {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdateSubscriptionPermission(
	...
	resolverBySid func(participantID hublive.ParticipantID) types.LocalParticipant,
) error {
	u.lock.Lock()
	...
	if err := u.parseSubscriptionPermissionsLocked(subscriptionPermission, func(pID hublive.ParticipantID) types.LocalParticipant {
		u.lock.Unlock()
...
func (u *UpTrackManager) SubscriptionPermission() (*hublive.SubscriptionPermission, utils.TimedVersion) {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) HasPermission(trackID hublive.TrackID, subIdentity hublive.ParticipantIdentity) bool {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdatePublishedAudioTrack(update *hublive.UpdateLocalAudioTrack) types.MediaTrack {
	track := u.GetPublishedTrack(hublive.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) UpdatePublishedVideoTrack(update *hublive.UpdateLocalVideoTrack) types.MediaTrack {
	track := u.GetPublishedTrack(hublive.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) getPublishedTrackLocked(trackID hublive.TrackID) types.MediaTrack {
	return u.publishedTracks[trackID]
}

func (u *UpTrackManager) parseSubscriptionPermissionsLocked(
	subscriptionPermission *hublive.SubscriptionPermission,
	resolver func(participantID hublive.ParticipantID) types.LocalParticipant,
) error {
	...
	subscriberPermissions := make(map[hublive.ParticipantIdentity]*hublive.TrackPermission)
	for _, trackPerms := range subscriptionPermission.TrackPermissions {
		subscriberIdentity := hublive.ParticipantIdentity(trackPerms.ParticipantIdentity)
		if subscriberIdentity == "" {
			if trackPerms.ParticipantSid == "" {
				return ErrSubscriptionPermissionNeedsId
			}

			sub := resolver(hublive.ParticipantID(trackPerms.ParticipantSid))
			if sub == nil {
...
func (u *UpTrackManager) hasPermissionLocked(trackID hublive.TrackID, subscriberIdentity hublive.ParticipantIdentity) bool {
	...
	for _, sid := range perms.TrackSids {
		if hublive.TrackID(sid) == trackID {
			return true
		}
	}
...
func (u *UpTrackManager) getAllowedSubscribersLocked(trackID hublive.TrackID) []hublive.ParticipantIdentity {
	if u.subscriberPermissions == nil {
		return nil
	}

	allowed := make([]hublive.ParticipantIdentity, 0)
...
func (u *UpTrackManager) DebugInfo() map[string]any {
	info := map[string]any{}
	publishedTrackInfo := make(map[hublive.TrackID]any)
...
func (u *UpTrackManager) GetAudioLevel() (level float64, active bool) {
	...
	for _, pt := range u.GetPublishedTracks() {
		if pt.Source() == hublive.TrackSource_MICROPHONE {
