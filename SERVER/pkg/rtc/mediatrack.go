...
import (
	...
	"github.com/maxhubsv/hublive-server/pkg/rtc/dynacast"
	...
	util "__GITHUB_HUBLIVE__mediatransportutil"
)
...
type MediaTrack struct {
	...
	*MediaLossProxy
	...
	backupCodecPolicy             hublive.BackupCodecPolicy
...
type MediaTrackParams struct {
	...
	OnTrackEverSubscribed            func(hublive.TrackID)
...
func NewMediaTrack(params MediaTrackParams, ti *hublive.TrackInfo) *MediaTrack {
	t := &MediaTrack{
		params:            params,
	...
	if t.params.ForceBackupCodecPolicySimulcast {
		t.backupCodecPolicy = hublive.BackupCodecPolicy_SIMULCAST
	}

	if t.backupCodecPolicy != hublive.BackupCodecPolicy_SIMULCAST && len(ti.Codecs) > 1 {
	...
	if ti.Type == hublive.TrackType_AUDIO {
	...
	t.MediaTrackReceiver.OnSubscriberMaxQualityChange(
		func(subscriberID hublive.ParticipantID, mimeType mime.MimeType, layer int32) {
			if t.dynacastManager != nil {
	...
	t.MediaTrackReceiver.OnSubscriberAudioCodecChange(
		func(subscriberID hublive.ParticipantID, mimeType mime.MimeType, enabled bool) {
			if t.dynacastManager != nil {
...
func (t *MediaTrack) NotifySubscriberNodeMaxQuality(nodeID hublive.NodeID, qualities []types.SubscribedCodecQuality) {
	if t.dynacastManager != nil {
		t.dynacastManager.NotifySubscriberNodeMaxQuality(nodeID, qualities)
	}
}

func (t *MediaTrack) NotifySubscriptionNode(nodeID hublive.NodeID, codecs []*hublive.SubscribedAudioCodec) {
	if t.dynacastManager != nil {
...
func (t *MediaTrack) ToProto() *hublive.TrackInfo {
...
func (t *MediaTrack) AddReceiver(receiver *webrtc.RTPReceiver, track sfu.TrackRemote, mid string) (bool, bool) {
		...
		for _, pkt := range pkts {
			...
			case *rtcp.SenderReport:
				if pkt.SSRC == uint32(track.SSRC()) {
					buff.SetSenderReportData(&hublive.RTCPSenderReportState{
	...
	if wr == nil {
		...
		statsKey := telemetry.StatsKeyForTrack(
			...
			hublive.StreamType_UPSTREAM,
			t.PublisherID(),
			t.ID(),
			ti.Source,
			ti.Type,
		)
		newWR.OnStatsUpdate(func(_ *sfu.WebRTCReceiver, stat *hublive.AnalyticsStat) {
	...
	buff.OnFinalRtpStats(func(stats *hublive.RTPStats) {
...
func (t *MediaTrack) GetConnectionScoreAndQuality() (float32, hublive.ConnectionQuality) {
	receiver := t.ActiveReceiver()
	if rtcReceiver, ok := receiver.(*sfu.WebRTCReceiver); ok {
		return rtcReceiver.GetConnectionScoreAndQuality()
	}

	return connectionquality.MaxMOS, hublive.ConnectionQuality_EXCELLENT
}
...
func (t *MediaTrack) enableRegression() bool {
	return t.backupCodecPolicy == hublive.BackupCodecPolicy_REGRESSION ||
		(t.backupCodecPolicy == hublive.BackupCodecPolicy_PREFER_REGRESSION && t.params.ShouldRegressCodec())
}
...
func (t *MediaTrack) OnDynacastSubscribedAudioCodecChange(codecs []*hublive.SubscribedAudioCodec) {
	t.lock.RLock()
