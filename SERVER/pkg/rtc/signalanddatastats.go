...
func (s *BytesTrackStats) report() {
	if recv := s.recv.Swap(0); recv > 0 {
		...
		s.telemetryListener.OnTrackStats(
			telemetry.StatsKeyForData(s.country, hublive.StreamType_UPSTREAM, s.pID, s.trackID),
	...
	if send := s.send.Swap(0); send > 0 {
		...
		s.telemetryListener.OnTrackStats(
			telemetry.StatsKeyForData(s.country, hublive.StreamType_DOWNSTREAM, s.pID, s.trackID),
			&hublive.AnalyticsStat{
				Streams: []*hublive.AnalyticsStream{
					{
						PrimaryBytes:   send,
...
func (s *BytesSignalStats) ResolveRoom(ri *hublive.Room) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *BytesSignalStats) ResolveParticipant(pi *hublive.ParticipantInfo) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.pi == nil && pi != nil {
		s.pi = &hublive.ParticipantInfo{
			Sid:      pi.Sid,
...
func (s *BytesSignalStats) maybeStart() {
	...
	s.pID = hublive.ParticipantID(s.pi.Sid)
	s.trackID = BytesTrackIDForParticipantID(BytesTrackTypeSignal, s.pID)

	s.participantResolver.Resolve(
		hublive.RoomName(s.ri.Name),
		hublive.RoomID(s.ri.Sid),
		hublive.ParticipantIdentity(s.pi.Identity),
		hublive.ParticipantID(s.pi.Sid),
	)
...
func (s *BytesSignalStats) OnTrackStats(key telemetry.StatsKey, stat *hublive.AnalyticsStat) {
	stat.RoomId, stat.RoomName = s.ri.Sid, s.ri.Name
	s.telemetry.TrackStats(hublive.RoomID(s.ri.Sid), hublive.RoomName(s.ri.Name), key, stat)
}

// -----------------------------------------------------------------------

func BytesTrackIDForParticipantID(typ BytesTrackType, participantID hublive.ParticipantID) hublive.TrackID {
	return hublive.TrackID(fmt.Sprintf("%s%s%s", utils.TrackPrefix, typ, participantID))
}
