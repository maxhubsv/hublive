...
type dynacastQualityAudio struct {
	...
	lock                  sync.RWMutex
	initialized           bool
	subscriberEnables     map[hublive.ParticipantID]bool
	subscriberNodeEnables map[hublive.NodeID]bool
	enabled               bool
...
func newDynacastQualityAudio(params dynacastQualityAudioParams) dynacastQuality {
	return &dynacastQualityAudio{
		params:                params,
		subscriberEnables:     make(map[hublive.ParticipantID]bool),
		subscriberNodeEnables: make(map[hublive.NodeID]bool),
	}
}
...
func (d *dynacastQualityAudio) NotifySubscription(subscriberID hublive.ParticipantID, enabled bool) {
...
func (d *dynacastQualityAudio) NotifySubscriptionNode(nodeID hublive.NodeID, enabled bool) {
	d.params.Logger.Debugw(
...
func (d *dynacastQualityAudio) ClearSubscriberNodes() {
	d.lock.Lock()
	d.subscriberNodeEnables = make(map[hublive.NodeID]bool)
	d.lock.Unlock()
