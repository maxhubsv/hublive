...
func (d *dynacastManagerAudio) enqueueSubscribedChange() {
	...
	subscribedCodecs := make([]*hublive.SubscribedAudioCodec, 0, len(d.committedSubscribedCodecs))
	for mime, enabled := range d.committedSubscribedCodecs {
		subscribedCodecs = append(subscribedCodecs, &hublive.SubscribedAudioCodec{
			Codec:   mime.String(),
			Enabled: enabled,
		})
	}
