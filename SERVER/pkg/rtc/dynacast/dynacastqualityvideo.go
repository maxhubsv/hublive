...
type dynacastQualityVideo struct {
	...
	maxSubscribedQuality     hublive.VideoQuality
...
func newDynacastQualityVideo(params dynacastQualityVideoParams) dynacastQuality {
	return &dynacastQualityVideo{
		params:                   params,
		maxSubscriberQuality:     make(map[hublive.ParticipantID]hublive.VideoQuality),
		maxSubscriberNodeQuality: make(map[hublive.NodeID]hublive.VideoQuality),
	}
}
...
func (d *dynacastQualityVideo) NotifySubscriberMaxQuality(subscriberID hublive.ParticipantID, quality hublive.VideoQuality) {
...
func (d *dynacastQualityVideo) NotifySubscriberNodeMaxQuality(nodeID hublive.NodeID, quality hublive.VideoQuality) {
	d.params.Logger.Debugw(
...
func (d *dynacastQualityVideo) ClearSubscriberNodes() {
	d.lock.Lock()
	d.maxSubscriberNodeQuality = make(map[hublive.NodeID]hublive.VideoQuality)
	d.lock.Unlock()
...
func (d *dynacastQualityVideo) RegressTo(other dynacastQuality) {
	...
	d.maxSubscriberQuality = make(map[hublive.ParticipantID]hublive.VideoQuality)
	d.maxSubscriberNodeQuality = make(map[hublive.NodeID]hublive.VideoQuality)
	d.lock.Unlock()
...
func (d *dynacastQualityVideo) updateQualityChange(force bool) {
	d.lock.Lock()
	maxSubscribedQuality := hublive.VideoQuality_OFF
	for _, subQuality := range d.maxSubscriberQuality {
		if maxSubscribedQuality == hublive.VideoQuality_OFF || (subQuality != hublive.VideoQuality_OFF && subQuality > maxSubscribedQuality) {
			maxSubscribedQuality = subQuality
		}
	}
	for _, nodeQuality := range d.maxSubscriberNodeQuality {
		if maxSubscribedQuality == hublive.VideoQuality_OFF || (nodeQuality != hublive.VideoQuality_OFF && nodeQuality > maxSubscribedQuality) {
