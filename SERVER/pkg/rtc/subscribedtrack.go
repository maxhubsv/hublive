...
func (t *SubscribedTrack) Bound(err error) {
	...
	if err == nil && t.MediaTrack().Kind() == hublive.TrackType_VIDEO {
		...
		if t.settings != nil {
			...
			if t.params.AdaptiveStream {
				t.settings = &hublive.UpdateTrackSettings{Quality: hublive.VideoQuality_LOW}
			} else {
				t.settings = &hublive.UpdateTrackSettings{Quality: hublive.VideoQuality_HIGH}
			}
...
func (t *SubscribedTrack) ID() hublive.TrackID {
	return hublive.TrackID(t.downTrack.ID())
}

func (t *SubscribedTrack) PublisherID() hublive.ParticipantID {
	return t.params.MediaTrack.PublisherID()
}

func (t *SubscribedTrack) PublisherIdentity() hublive.ParticipantIdentity {
...
func (t *SubscribedTrack) SubscriberID() hublive.ParticipantID {
	return t.params.Subscriber.ID()
}

func (t *SubscribedTrack) SubscriberIdentity() hublive.ParticipantIdentity {
...
func (t *SubscribedTrack) UpdateSubscriberSettings(settings *hublive.UpdateTrackSettings, isImmediate bool) {
	t.settingsLock.Lock()
...
func (t *SubscribedTrack) OnStatsUpdate(stat *hublive.AnalyticsStat) {
