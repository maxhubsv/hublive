...
func (d *DataDownTrack) ID() hublive.TrackID {
...
func (d *DataDownTrack) SubscriberID() hublive.ParticipantID {
	// add `createdAt` to ensure repeated subscriptions from same subscriber to same publisher does not collide
	return hublive.ParticipantID(fmt.Sprintf("%s:%d", d.params.SubscriberID, d.createdAt))
}
...
func (d *DataDownTrack) UpdateSubscriptionOptions(subscriptionOptions *hublive.DataTrackSubscriptionOptions) {
	// DT-TODO
}
