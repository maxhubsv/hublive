...
type MediaTrackSubscriptions struct {
	...
	subscribedTracks   map[hublive.ParticipantID]types.SubscribedTrack

	onDownTrackCreated           func(downTrack *sfu.DownTrack)
	onSubscriberMaxQualityChange func(subscriberID hublive.ParticipantID, mime mime.MimeType, layer int32)
	onSubscriberAudioCodecChange func(subscriberID hublive.ParticipantID, mime mime.MimeType, enabled bool)
}
...
func NewMediaTrackSubscriptions(params MediaTrackSubscriptionsParams) *MediaTrackSubscriptions {
	return &MediaTrackSubscriptions{
		params:           params,
		subscribedTracks: make(map[hublive.ParticipantID]types.SubscribedTrack),
	}
}
...
func (t *MediaTrackSubscriptions) OnSubscriberMaxQualityChange(f func(subscriberID hublive.ParticipantID, mime mime.MimeType, layer int32)) {
	t.onSubscriberMaxQualityChange = f
}

func (t *MediaTrackSubscriptions) OnSubscriberAudioCodecChange(f func(subscriberID hublive.ParticipantID, mime mime.MimeType, enabled bool)) {
...
func (t *MediaTrackSubscriptions) IsSubscriber(subID hublive.ParticipantID) bool {
...
func (t *MediaTrackSubscriptions) AddSubscriber(sub types.LocalParticipant, wr *WrappedReceiver) (types.SubscribedTrack, error) {
	...
	subTrack, err := NewSubscribedTrack(SubscribedTrackParams{
		...
		OnDownTrackClosed: func(subscriberID hublive.ParticipantID) {
			t.subscribedTracksMu.Lock()
	...
	if transceiver == nil {
		...
		addTrackParams := types.AddTrackParams{
			Stereo: slices.Contains(info.AudioFeatures, hublive.AudioTrackFeature_TF_STEREO),
...
func (t *MediaTrackSubscriptions) RemoveSubscriber(subscriberID hublive.ParticipantID, isExpectedToResume bool) error {
...
func (t *MediaTrackSubscriptions) GetAllSubscribers() []hublive.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()

	subs := make([]hublive.ParticipantID, 0, len(t.subscribedTracks))
...
func (t *MediaTrackSubscriptions) GetAllSubscribersForMime(mime mime.MimeType) []hublive.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
...
func (t *MediaTrackSubscriptions) getSubscribedTrack(subscriberID hublive.ParticipantID) types.SubscribedTrack {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
