...
func TestPackStreamId(t *testing.T) {
	...
	require.Equal(t, hublive.ParticipantID("PA_123abc"), pID)
	require.Equal(t, hublive.TrackID("uuid-id"), trackID)

	require.Equal(t, packed, PackStreamID(pID, trackID))
}

func TestPackDataTrackLabel(t *testing.T) {
	pID := hublive.ParticipantID("PA_123abc")
	trackID := hublive.TrackID("TR_b3da25")
	label := "trackLabel"
...
func TestChunkProtoBatch(t *testing.T) {
	...
	var updates []*hublive.ParticipantInfo
	for range 32 {
		updates = append(updates, &hublive.ParticipantInfo{
