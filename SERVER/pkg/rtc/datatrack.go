...
type DataTrack struct {
	...
	lock             sync.Mutex
	dti              *hublive.DataTrackInfo
	subscribedTracks map[hublive.ParticipantID]types.DataDownTrack
...
func NewDataTrack(params DataTrackParams, dti *hublive.DataTrackInfo) *DataTrack {
	d := &DataTrack{
		params:           params,
		dti:              dti,
		subscribedTracks: make(map[hublive.ParticipantID]types.DataDownTrack),
...
func (d *DataTrack) PublisherID() hublive.ParticipantID {
	return d.params.ParticipantID()
}

func (d *DataTrack) PublisherIdentity() hublive.ParticipantIdentity {
	return d.params.ParticipantIdentity
}

func (d *DataTrack) ToProto() *hublive.DataTrackInfo {
...
func (d *DataTrack) ID() hublive.TrackID {
	return hublive.TrackID(d.dti.Sid)
}
...
func (d *DataTrack) RemoveSubscriber(subID hublive.ParticipantID) {
	d.lock.Lock()
...
func (d *DataTrack) IsSubscriber(subID hublive.ParticipantID) bool {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func (d *DataTrack) DeleteDataDownTrack(subscriberID hublive.ParticipantID) {
