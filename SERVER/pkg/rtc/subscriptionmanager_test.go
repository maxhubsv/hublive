...
func TestSubscribe(t *testing.T) {
	t.Run("happy path subscribe", func(t *testing.T) {
		...
		sm.params.OnSubscriptionError = func(trackID hublive.TrackID, fatal bool, err error) {
			failed.Store(true)
		}
		numParticipantSubscribed := atomic.Int32{}
		numParticipantUnsubscribed := atomic.Int32{}
		sm.OnSubscribeStatusChanged(func(pubID hublive.ParticipantID, subscribed bool) {
			if subscribed {
...
func TestUnsubscribe(t *testing.T) {
	...
	res.Track.(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID hublive.ParticipantID, isExpectedToResume bool) {
...
func TestSubscribeStatusChanged(t *testing.T) {
	...
	st1.MediaTrack().(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID hublive.ParticipantID, isExpectedToResume bool) {
		setTestSubscribedTrackClosed(t, st1, isExpectedToResume)
	})
	st2.MediaTrack().(*typesfakes.FakeMediaTrack).RemoveSubscriberCalls(func(pID hublive.ParticipantID, isExpectedToResume bool) {
...
func newTestSubscriptionManagerWithParams(params testSubscriptionParams) *SubscriptionManager {
	...
	p.KindReturns(hublive.ParticipantInfo_STANDARD)
	return NewSubscriptionManager(SubscriptionManagerParams{
		...
		TrackResolver: func(sub types.LocalParticipant, trackID hublive.TrackID) types.MediaResolverResult {
...
type testResolver struct {
	...
	hasPermission bool
	hasTrack      bool
	pubIdentity   hublive.ParticipantIdentity
	pubID         hublive.ParticipantID

	paused bool
}

func newTestResolver(hasPermission bool, hasTrack bool, pubIdentity hublive.ParticipantIdentity, pubID hublive.ParticipantID) *testResolver {
	return &testResolver{
...
func (t *testResolver) Resolve(_subscriber types.LocalParticipant, trackID hublive.TrackID) types.MediaResolverResult {
	t.lock.Lock()
	defer t.lock.Unlock()
