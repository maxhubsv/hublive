...
func TestUpdateSubscriptionPermission(t *testing.T) {
	t.Run("updates subscription permission", func(t *testing.T) {
		...
		subscriptionPermission = &hublive.SubscriptionPermission{
			TrackPermissions: []*hublive.TrackPermission{},
		}
		um.UpdateSubscriptionPermission(subscriptionPermission, vg.Next(), nil)
		...
		sidResolver := func(sid hublive.ParticipantID) types.LocalParticipant {
			if sid == "p1" {
				return lp1
			}
		...
		perms1 := &hublive.TrackPermission{
			ParticipantSid: "p1",
			AllTracks:      true,
		}
		perms2 := &hublive.TrackPermission{
			ParticipantSid: "p2",
			AllTracks:      true,
		}
		subscriptionPermission = &hublive.SubscriptionPermission{
		...
		perms3 := &hublive.TrackPermission{
	...
	t.Run("updates subscription permission using both", func(t *testing.T) {
		...
		badSidResolver := func(sid hublive.ParticipantID) types.LocalParticipant {
			if sid == "p1" {
				return lp2
			}
	...
	t.Run("update versions", func(t *testing.T) {
		...
		um.UpdateSubscriptionPermission(&hublive.SubscriptionPermission{}, v1, nil)
		require.Equal(t, v1.Load(), um.subscriptionPermissionVersion.Load(), "first update should be applied")

		um.UpdateSubscriptionPermission(&hublive.SubscriptionPermission{}, v2, nil)
		require.Equal(t, v2.Load(), um.subscriptionPermissionVersion.Load(), "ordered updates should be applied")

		um.UpdateSubscriptionPermission(&hublive.SubscriptionPermission{}, v0, nil)
		require.Equal(t, v2.Load(), um.subscriptionPermissionVersion.Load(), "out of order updates should be ignored")

		um.UpdateSubscriptionPermission(&hublive.SubscriptionPermission{}, utils.TimedVersion(0), nil)
