...
func (p *ParticipantImpl) SendJoinResponse(joinResponse *hublive.JoinResponse) error {
	// keep track of participant updates and versions
	p.updateLock.Lock()
	for _, op := range joinResponse.OtherParticipants {
		p.updateCache.Add(hublive.ParticipantID(op.Sid), participantUpdateInfo{
			identity:  hublive.ParticipantIdentity(op.Identity),
			version:   op.Version,
			state:     op.State,
			updatedAt: time.Now(),
		})
	...
	p.updateLock.Lock()
	if p.State() == hublive.ParticipantInfo_JOINING {
...
func (p *ParticipantImpl) SendParticipantUpdate(participantsToUpdate []*hublive.ParticipantInfo) error {
	p.updateLock.Lock()
	if p.IsDisconnected() {
		p.updateLock.Unlock()
		return nil
	...
	validUpdates := make([]*hublive.ParticipantInfo, 0, len(participantsToUpdate))
	for _, pi := range participantsToUpdate {
		isValid := true
		pID := hublive.ParticipantID(pi.Sid)
...
func (p *ParticipantImpl) SendSpeakerUpdate(speakers []*hublive.SpeakerInfo, force bool) error {
	if !p.IsReady() {
		return nil
	}

	var scopedSpeakers []*hublive.SpeakerInfo
	if force {
		scopedSpeakers = speakers
	} else {
		for _, s := range speakers {
			participantID := hublive.ParticipantID(s.Sid)
...
func (p *ParticipantImpl) SendRoomUpdate(room *hublive.Room) error {
	return p.signaller.WriteMessage(p.signalling.SignalRoomUpdate(room))
}

func (p *ParticipantImpl) SendConnectionQualityUpdate(update *hublive.ConnectionQualityUpdate) error {
...
func (p *ParticipantImpl) sendRequestResponse(requestResponse *hublive.RequestResponse) error {
	if !p.params.ClientInfo.SupportsRequestResponse() {
		return nil
	}

	if requestResponse.Reason == hublive.RequestResponse_OK && !p.ProtocolVersion().SupportsNonErrorSignalResponse() {
		return nil
	}
...
func (p *ParticipantImpl) SendRoomMovedResponse(roomMovedResponse *hublive.RoomMovedResponse) error {
	return p.signaller.WriteMessage(p.signalling.SignalRoomMovedResponse(roomMovedResponse))
}

func (p *ParticipantImpl) HandleReconnectAndSendResponse(reconnectReason hublive.ReconnectReason, reconnectResponse *hublive.ReconnectResponse) error {
...
func (p *ParticipantImpl) sendDisconnectUpdatesForReconnect() error {
	...
	var disconnectedParticipants []*hublive.ParticipantInfo
	p.updateLock.Lock()
	...
	for i := len(keys) - 1; i >= 0; i-- {
		if info, ok := p.updateCache.Get(keys[i]); ok {
			if info.updatedAt.Before(lastSignalAt) {
				break
			} else if info.state == hublive.ParticipantInfo_DISCONNECTED {
				disconnectedParticipants = append(disconnectedParticipants, &hublive.ParticipantInfo{
					Sid:      string(keys[i]),
...
func (p *ParticipantImpl) sendICECandidate(ic *webrtc.ICECandidate, target hublive.SignalTarget) error {
...
func (p *ParticipantImpl) sendTrackMuted(trackID hublive.TrackID, muted bool) {
	_ = p.signaller.WriteMessage(p.signalling.SignalTrackMuted(&hublive.MuteTrackRequest{
		Sid:   string(trackID),
		Muted: muted,
	}))
}

func (p *ParticipantImpl) sendTrackPublished(cid string, ti *hublive.TrackInfo) error {
	p.pubLogger.Debugw("sending track published", "cid", cid, "trackInfo", logger.Proto(ti))
	return p.signaller.WriteMessage(p.signalling.SignalTrackPublished(&hublive.TrackPublishedResponse{
		Cid:   cid,
		Track: ti,
	}))
}

func (p *ParticipantImpl) sendTrackUnpublished(trackID hublive.TrackID) {
	_ = p.signaller.WriteMessage(p.signalling.SignalTrackUnpublished(&hublive.TrackUnpublishedResponse{
		TrackSid: string(trackID),
	}))
}

func (p *ParticipantImpl) sendTrackHasBeenSubscribed(trackID hublive.TrackID) {
	if !p.params.ClientInfo.SupportsTrackSubscribedEvent() {
		return
	}
	_ = p.signaller.WriteMessage(p.signalling.SignalTrackSubscribed(&hublive.TrackSubscribed{
		TrackSid: string(trackID),
	}))
...
func (p *ParticipantImpl) sendLeaveRequest(
	...
	if p.ProtocolVersion().SupportsRegionsInLeaveRequest() {
		...
		case isExpectedToResume:
			leave.Action = hublive.LeaveRequest_RESUME
		case isExpectedToReconnect:
			leave.Action = hublive.LeaveRequest_RECONNECT
		default:
			leave.Action = hublive.LeaveRequest_DISCONNECT
		}
		if leave.Action != hublive.LeaveRequest_DISCONNECT {
...
func (p *ParticipantImpl) sendStreamStateUpdate(streamStateUpdate *hublive.StreamStateUpdate) error {
	return p.signaller.WriteMessage(p.signalling.SignalStreamStateUpdate(streamStateUpdate))
}

func (p *ParticipantImpl) sendSubscribedQualityUpdate(subscribedQualityUpdate *hublive.SubscribedQualityUpdate) error {
	return p.signaller.WriteMessage(p.signalling.SignalSubscribedQualityUpdate(subscribedQualityUpdate))
}

func (p *ParticipantImpl) sendSubscribedAudioCodecUpdate(subscribedAudioCodecUpdate *hublive.SubscribedAudioCodecUpdate) error {
	return p.signaller.WriteMessage(p.signalling.SignalSubscribedAudioCodecUpdate(subscribedAudioCodecUpdate))
}

func (p *ParticipantImpl) sendSubscriptionResponse(trackID hublive.TrackID, subErr hublive.SubscriptionError) error {
	return p.signaller.WriteMessage(p.signalling.SignalSubscriptionResponse(&hublive.SubscriptionResponse{
		TrackSid: string(trackID),
		Err:      subErr,
	}))
}

func (p *ParticipantImpl) SendSubscriptionPermissionUpdate(publisherID hublive.ParticipantID, trackID hublive.TrackID, allowed bool) error {
	p.subLogger.Debugw("sending subscription permission update", "publisherID", publisherID, "trackID", trackID, "allowed", allowed)
	err := p.signaller.WriteMessage(p.signalling.SignalSubscriptionPermissionUpdate(&hublive.SubscriptionPermissionUpdate{
...
func (p *ParticipantImpl) sendMediaSectionsRequirement(numAudios uint32, numVideos uint32) error {
	...
	err := p.signaller.WriteMessage(p.signalling.SignalMediaSectionsRequirement(&hublive.MediaSectionsRequirement{
		NumAudios: numAudios,
		NumVideos: numVideos,
	}))
	if err != nil {
...
func (p *ParticipantImpl) sendPublishDataTrackResponse(dti *hublive.DataTrackInfo) error {
	return p.signaller.WriteMessage(p.signalling.SignalPublishDataTrackResponse(&hublive.PublishDataTrackResponse{
		Info: dti,
	}))
}

func (p *ParticipantImpl) sendUnpublishDataTrackResponse(dti *hublive.DataTrackInfo) error {
	return p.signaller.WriteMessage(p.signalling.SignalUnpublishDataTrackResponse(&hublive.UnpublishDataTrackResponse{
		Info: dti,
	}))
}

func (p *ParticipantImpl) SendDataTrackSubscriberHandles(handles map[uint32]*hublive.DataTrackSubscriberHandles_PublishedDataTrack) error {
	return p.signaller.WriteMessage(p.signalling.SignalDataTrackSubscriberHandles(&hublive.DataTrackSubscriberHandles{
		SubHandles: handles,
	}))
}
