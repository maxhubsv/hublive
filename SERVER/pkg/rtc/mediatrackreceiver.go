...
import (
	...
	"github.com/maxhubsv/hublive-server/pkg/sfu/rtpstats"
...
type MediaTrackReceiverParams struct {
	...
	IsRelayed                bool
	ParticipantID            func() hublive.ParticipantID
...
type MediaTrackReceiver struct {
	...
	trackInfo          atomic.Pointer[hublive.TrackInfo]
...
func NewMediaTrackReceiver(params MediaTrackReceiverParams, ti *hublive.TrackInfo) *MediaTrackReceiver {
...
func (t *MediaTrackReceiver) Restart() {
	for _, receiver := range t.loadReceivers() {
		hq := buffer.GetSpatialLayerForVideoQuality(receiver.Mime(), hublive.VideoQuality_HIGH, t.TrackInfo())
...
func (t *MediaTrackReceiver) ID() hublive.TrackID {
	return hublive.TrackID(t.TrackInfo().Sid)
}

func (t *MediaTrackReceiver) Kind() hublive.TrackType {
	return t.TrackInfo().Type
}

func (t *MediaTrackReceiver) Source() hublive.TrackSource {
...
func (t *MediaTrackReceiver) PublisherID() hublive.ParticipantID {
	return t.params.ParticipantID()
}

func (t *MediaTrackReceiver) PublisherIdentity() hublive.ParticipantIdentity {
...
func (t *MediaTrackReceiver) IsEncrypted() bool {
	return t.TrackInfo().Encryption != hublive.Encryption_NONE
}
...
func (t *MediaTrackReceiver) RemoveSubscriber(subscriberID hublive.ParticipantID, isExpectedToResume bool) {
...
func (t *MediaTrackReceiver) RevokeDisallowedSubscribers(allowedSubscriberIdentities []hublive.ParticipantIdentity) []hublive.ParticipantIdentity {
	var revokedSubscriberIdentities []hublive.ParticipantIdentity
...
func (t *MediaTrackReceiver) SetLayerSsrcsForRid(mimeType mime.MimeType, rid string, ssrc uint32, repairSSRC uint32) {
	...
	for i, ci := range trackInfo.Codecs {
		...
		var matchingLayer *hublive.VideoLayer
		ssrcFound := false
...
func (t *MediaTrackReceiver) UpdateCodecInfo(codecs []*hublive.SimulcastCodec) {
	t.lock.Lock()
	trackInfo := t.TrackInfoClone()
	for _, c := range codecs {
		for _, origin := range trackInfo.Codecs {
			if mime.GetMimeTypeCodec(origin.MimeType) == mime.NormalizeMimeTypeCodec(c.Codec) {
				...
				if len(c.Layers) != 0 {
					clonedLayers := make([]*hublive.VideoLayer, 0, len(c.Layers))
...
func (t *MediaTrackReceiver) UpdateTrackInfo(ti *hublive.TrackInfo) {
	updateMute := false
...
func (t *MediaTrackReceiver) UpdateAudioTrack(update *hublive.UpdateLocalAudioTrack) {
	if t.Kind() != hublive.TrackType_AUDIO {
		return
	}
...
func (t *MediaTrackReceiver) UpdateVideoTrack(update *hublive.UpdateLocalVideoTrack) {
	if t.Kind() != hublive.TrackType_VIDEO {
		return
	}
...
func (t *MediaTrackReceiver) TrackInfo() *hublive.TrackInfo {
	return t.trackInfo.Load()
}

func (t *MediaTrackReceiver) TrackInfoClone() *hublive.TrackInfo {
...
func (t *MediaTrackReceiver) NotifyMaxLayerChange(mimeType mime.MimeType, maxLayer int32) {
	...
	ti := &hublive.TrackInfo{
		Sid:    trackInfo.Sid,
		Type:   trackInfo.Type,
		Layers: []*hublive.VideoLayer{{Quality: quality}},
	}
...
func (t *MediaTrackReceiver) GetQualityForDimension(mimeType mime.MimeType, width, height uint32) hublive.VideoQuality {
	...
	for i, s := range layerSizes {
		quality = hublive.VideoQuality(i)
...
func (t *MediaTrackReceiver) GetTrackStats() *hublive.RTPStats {
	receivers := t.loadReceivers()
	stats := make([]*hublive.RTPStats, 0, len(receivers))
	for _, receiver := range receivers {
