...
const (
	trackIDForReconcileSubscriptions = hublive.TrackID("subscriptions_reconcile")
)
...
type SubscriptionManagerParams struct {
	...
	OnSubscriptionError func(trackID hublive.TrackID, fatal bool, err error)
...
type SubscriptionManager struct {
	...
	lock                sync.RWMutex
	subscriptions       map[hublive.TrackID]*mediaTrackSubscription
	pendingUnsubscribes atomic.Int32

	subscribedVideoCount, subscribedAudioCount atomic.Int32

	subscribedTo         map[hublive.ParticipantID]map[hublive.TrackID]struct{}
	reconcileCh          chan hublive.TrackID
	reconcileDataTrackCh chan hublive.TrackID
	closeCh              chan struct{}
	doneCh               chan struct{}

	onSubscribeStatusChanged func(publisherID hublive.ParticipantID, subscribed bool)

	dataTrackSubscriptions map[hublive.TrackID]*dataTrackSubscription
}

func NewSubscriptionManager(params SubscriptionManagerParams) *SubscriptionManager {
	m := &SubscriptionManager{
		params:                 params,
		subscriptions:          make(map[hublive.TrackID]*mediaTrackSubscription),
		subscribedTo:           make(map[hublive.ParticipantID]map[hublive.TrackID]struct{}),
		reconcileCh:            make(chan hublive.TrackID, 50),
		reconcileDataTrackCh:   make(chan hublive.TrackID, 5),
		closeCh:                make(chan struct{}),
		doneCh:                 make(chan struct{}),
		dataTrackSubscriptions: make(map[hublive.TrackID]*dataTrackSubscription),
	}
...
func (m *SubscriptionManager) SubscribeToTrack(trackID hublive.TrackID, isSync bool) {
...
func (m *SubscriptionManager) UnsubscribeFromTrack(trackID hublive.TrackID) {
...
func (m *SubscriptionManager) SubscribeToDataTrack(trackID hublive.TrackID) {
...
func (m *SubscriptionManager) UnsubscribeFromDataTrack(trackID hublive.TrackID) {
...
func (m *SubscriptionManager) IsTrackNameSubscribed(publisherIdentity hublive.ParticipantIdentity, trackName string) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) StopAndGetSubscribedTracksForwarderState() map[hublive.TrackID]*hublive.RTPForwarderState {
	m.lock.RLock()
	defer m.lock.RUnlock()

	states := make(map[hublive.TrackID]*hublive.RTPForwarderState, len(m.subscriptions))
...
func (m *SubscriptionManager) GetSubscribedParticipants() []hublive.ParticipantID {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) IsSubscribedTo(participantID hublive.ParticipantID) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) UpdateSubscribedTrackSettings(trackID hublive.TrackID, settings *hublive.UpdateTrackSettings) {
	m.lock.Lock()
...
func (m *SubscriptionManager) UpdateDataTrackSubscriptionOptions(trackID hublive.TrackID, subscriptionOptions *hublive.DataTrackSubscriptionOptions) {
	m.lock.Lock()
...
func (m *SubscriptionManager) OnSubscribeStatusChanged(fn func(publisherID hublive.ParticipantID, subscribed bool)) {
	m.lock.Lock()
...
func (m *SubscriptionManager) setDesired(trackID hublive.TrackID, desired bool) (*mediaTrackSubscription, bool) {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) setDataTrackDesired(trackID hublive.TrackID, desired bool) (*dataTrackSubscription, bool) {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) queueReconcile(trackID hublive.TrackID) {
	select {
...
func (m *SubscriptionManager) queueReconcileDataTrack(trackID hublive.TrackID) {
	select {
...
func (m *SubscriptionManager) hasCapacityForSubscription(kind hublive.TrackType) bool {
	switch kind {
...
func (m *SubscriptionManager) subscribeSynchronous(trackID hublive.TrackID) error {
...
func (m *SubscriptionManager) unsubscribeSynchronous(trackID hublive.TrackID) error {
	m.lock.Lock()
...
func (m *SubscriptionManager) handleSourceTrackRemoved(trackID hublive.TrackID) {
	m.lock.Lock()
	sub := m.subscriptions[trackID]
	m.lock.Unlock()
...
func (m *SubscriptionManager) handleSubscribedTrackClose(s *mediaTrackSubscription, isExpectedToResume bool) {
	...
	if wasBound {
		m.params.TelemetryListener.OnTrackUnsubscribed(
			s.subscriberID,
			&hublive.TrackInfo{Sid: string(s.trackID), Type: subTrack.MediaTrack().Kind()},
			!isExpectedToResume,
		)
...
func (m *SubscriptionManager) notifyDataTrackSubscriberHandles() {
	m.lock.Lock()
	handles := make(map[uint32]*hublive.DataTrackSubscriberHandles_PublishedDataTrack, len(m.dataTrackSubscriptions))
	for _, sub := range m.dataTrackSubscriptions {
		if !sub.isDesired() {
			continue
		}
		...
		handles[uint32(dataDownTrack.Handle())] = &hublive.DataTrackSubscriberHandles_PublishedDataTrack{
...
func (m *SubscriptionManager) handleSourceDataTrackRemoved(trackID hublive.TrackID) {
	m.lock.Lock()
...
func (m *SubscriptionManager) markSubscribedTo(publisherID hublive.ParticipantID, trackID hublive.TrackID) {
	...
	if pTracks == nil {
		pTracks = make(map[hublive.TrackID]struct{})
...
func (m *SubscriptionManager) unmarkSubscribedTo(publisherID hublive.ParticipantID, trackID hublive.TrackID) {
...
func (s *trackSubscription) setPublisher(publisherIdentity hublive.ParticipantIdentity, publisherID hublive.ParticipantID) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *trackSubscription) getPublisherID() hublive.ParticipantID {
	s.lock.RLock()
...
type mediaTrackSubscription struct {
	...
	settings                 *hublive.UpdateTrackSettings
	hasPermissionInitialized bool
	hasPermission            bool
	subscribedTrack          types.SubscribedTrack
	eventSent                atomic.Bool
	bound                    bool
	kind                     atomic.Pointer[hublive.TrackType]

	succRecordCounter atomic.Int32
}

func newMediaTrackSubscription(subscriberID hublive.ParticipantID, trackID hublive.TrackID, l logger.Logger) *mediaTrackSubscription {
...
func (s *mediaTrackSubscription) trySetKind(kind hublive.TrackType) {
	s.kind.CompareAndSwap(nil, &kind)
}

func (s *mediaTrackSubscription) getKind() (hublive.TrackType, bool) {
	kind := s.kind.Load()
	if kind == nil {
		return hublive.TrackType_AUDIO, false
	}
	return *kind, true
}

func (s *mediaTrackSubscription) setSettings(settings *hublive.UpdateTrackSettings) {
	s.lock.Lock()
	s.settings = settings
...
type dataTrackSubscription struct {
	...
	subscriptionOptions *hublive.DataTrackSubscriptionOptions

	dataDownTrack types.DataDownTrack
}

func newDataTrackSubscription(subscriberID hublive.ParticipantID, trackID hublive.TrackID, l logger.Logger) *dataTrackSubscription {
...
func (s *dataTrackSubscription) setSubscriptionOptions(subscriptionOptions *hublive.DataTrackSubscriptionOptions) {
	s.lock.Lock()
