...
type ParticipantSupervisor struct {
	...
	publications         map[hublive.TrackID]*trackMonitor

	isStopped atomic.Bool

	onPublicationError func(trackID hublive.TrackID)
}

func NewParticipantSupervisor(params ParticipantSupervisorParams) *ParticipantSupervisor {
	p := &ParticipantSupervisor{
		params:       params,
		publications: make(map[hublive.TrackID]*trackMonitor),
	}
...
func (p *ParticipantSupervisor) OnPublicationError(f func(trackID hublive.TrackID)) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *ParticipantSupervisor) getOnPublicationError() func(trackID hublive.TrackID) {
	p.lock.RLock()
...
func (p *ParticipantSupervisor) AddPublication(trackID hublive.TrackID) {
	p.lock.Lock()
...
func (p *ParticipantSupervisor) SetPublicationMute(trackID hublive.TrackID, isMuted bool) {
	p.lock.Lock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) SetPublishedTrack(trackID hublive.TrackID, pubTrack types.LocalMediaTrack) {
	p.lock.RLock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) ClearPublishedTrack(trackID hublive.TrackID, pubTrack types.LocalMediaTrack) {
	p.lock.RLock()
	pm, ok := p.publications[trackID]
	if ok {
...
func (p *ParticipantSupervisor) checkPublications() {
	var erroredPublications []hublive.TrackID
	var removablePublications []hublive.TrackID
	p.lock.RLock()
