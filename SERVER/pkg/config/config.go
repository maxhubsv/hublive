...
import (
	...
	redisHubLive "__GITHUB_HUBLIVE__protocol/redis"
...
type Config struct {
	...
	Redis          redisHubLive.RedisConfig `yaml:"redis,omitempty"`
...
type RoomConfig struct {
	...
	RoomConfigurations           map[string]*hublive.RoomConfiguration `yaml:"room_configurations,omitempty"`
}
...
var DefaultConfig = Config{
	...
	Redis: redisHubLive.RedisConfig{},
	Room: RoomConfig{
		AutoCreate: true,
...
func GenerateCLIFlags(existingFlags []cli.Flag, hidden bool) ([]cli.Flag, error) {
	...
	for name, value := range (defaultConfig).ToCLIFlagNames(existingFlags) {
		...
		var flag cli.Flag
		envVar := fmt.Sprintf("HUBLIVE_%s", strings.ToUpper(strings.Replace(name, ".", "_", -1)))
...
func SetLogger(l logger.Logger) {
	logger.SetLogger(l, "hublive")
}

func InitLoggerFromConfig(config *LoggingConfig) {
	logger.InitFromConfig(&config.Config, "hublive")
}
