...
func (r *StandardRoomAllocator) CreateRoom(ctx context.Context, req *hublive.CreateRoomRequest, isExplicit bool) (*hublive.Room, *hublive.RoomInternal, bool, error) {
	token, err := r.roomStore.LockRoom(ctx, hublive.RoomName(req.Name), 5*time.Second)
	if err != nil {
		return nil, nil, false, err
	}
	defer func() {
		_ = r.roomStore.UnlockRoom(ctx, hublive.RoomName(req.Name), token)
	}()

	// find existing room and update it
	var created bool
	rm, internal, err := r.roomStore.LoadRoom(ctx, hublive.RoomName(req.Name), true)
	...
	if req.MinPlayoutDelay > 0 || req.MaxPlayoutDelay > 0 {
		internal.PlayoutDelay = &hublive.PlayoutDelay{
			Enabled: true,
...
func (r *StandardRoomAllocator) SelectRoomNode(ctx context.Context, roomName hublive.RoomName, nodeID hublive.NodeID) error {
...
func (r *StandardRoomAllocator) ValidateCreateRoom(ctx context.Context, roomName hublive.RoomName) error {
...
func applyDefaultRoomConfig(room *hublive.Room, internal *hublive.RoomInternal, conf *config.RoomConfig) {
	...
	for _, codec := range conf.EnabledCodecs {
		room.EnabledCodecs = append(room.EnabledCodecs, &hublive.Codec{
			Mime:     codec.Mime,
...
func (r *StandardRoomAllocator) applyNamedRoomConfiguration(req *hublive.CreateRoomRequest) (*hublive.CreateRoomRequest, error) {
	if req.RoomPreset == "" {
		return req, nil
	}
	...
	if clone.Agents == nil {
		clone.Agents = make([]*hublive.RoomAgentDispatch, 0, len(conf.Agents))
