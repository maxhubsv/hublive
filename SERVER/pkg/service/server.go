...
import (
	...
	"__GITHUB_HUBLIVE__protocol/utils/xtwirp"
...
func NewHubliveServer(conf *config.Config,
	...
	roomServer := hublive.NewRoomServiceServer(roomService, serverOptions...)
	agentDispatchServer := hublive.NewAgentDispatchServiceServer(agentDispatchService, serverOptions...)
	egressServer := hublive.NewEgressServer(egressService, serverOptions...)
	ingressServer := hublive.NewIngressServer(ingressService, serverOptions...)
	sipServer := hublive.NewSIPServer(sipService, serverOptions...)
...
func (s *HubliveServer) Node() *hublive.Node {
	return s.currentNode.Clone()
}

func (s *HubliveServer) HTTPPort() int {
	return int(s.config.Port)
}

func (s *HubliveServer) IsRunning() bool {
	return s.running.Load()
}

func (s *HubliveServer) Start() error {
	if s.running.Load() {
	...
	logger.Infow("starting HubLive server", values...)
...
func (s *HubliveServer) Stop(force bool) {
...
func (s *HubliveServer) RoomManager() *RoomManager {
	return s.roomManager
}

func (s *HubliveServer) debugGoroutines(w http.ResponseWriter, _ *http.Request) {
	_ = pprof.Lookup("goroutine").WriteTo(w, 2)
}

func (s *HubliveServer) debugInfo(w http.ResponseWriter, _ *http.Request) {
...
func (s *HubliveServer) defaultHandler(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path == "/" {
		s.healthCheck(w, r)
	} else {
		http.NotFound(w, r)
...
func (s *HubliveServer) healthCheck(w http.ResponseWriter, _ *http.Request) {
...
func (s *HubliveServer) backgroundWorker() {
