...
import (
	...
	"__GITHUB_HUBLIVE__psrpc/pkg/middleware/otelpsrpc"
...
func (s *IOInfoService) CreateEgress(ctx context.Context, info *hublive.EgressInfo) (*emptypb.Empty, error) {
	if s.es == nil {
...
func (s *IOInfoService) UpdateEgress(ctx context.Context, info *hublive.EgressInfo) (*emptypb.Empty, error) {
	if s.es == nil {
		return nil, ErrEgressNotConnected
	}
	...
	switch info.Status {
	case hublive.EgressStatus_EGRESS_ACTIVE,
		hublive.EgressStatus_EGRESS_ENDING:
		s.telemetry.EgressUpdated(ctx, info)

	case hublive.EgressStatus_EGRESS_COMPLETE,
		hublive.EgressStatus_EGRESS_FAILED,
		hublive.EgressStatus_EGRESS_ABORTED,
		hublive.EgressStatus_EGRESS_LIMIT_REACHED:
...
func (s *IOInfoService) GetEgress(ctx context.Context, req *rpc.GetEgressRequest) (*hublive.EgressInfo, error) {
	if s.es == nil {
		return nil, ErrEgressNotConnected
	}
...
func (s *IOInfoService) ListEgress(ctx context.Context, req *hublive.ListEgressRequest) (*hublive.ListEgressResponse, error) {
	if s.es == nil {
		return nil, ErrEgressNotConnected
	}
	...
	if req.EgressId != "" {
		...
		return &hublive.ListEgressResponse{Items: []*hublive.EgressInfo{info}}, nil
	}

	items, err := s.es.ListEgress(ctx, hublive.RoomName(req.RoomName), req.Active)
	if err != nil {
		logger.Errorw("failed to list egress", err)
		return nil, err
	}

	return &hublive.ListEgressResponse{Items: items}, nil
}
