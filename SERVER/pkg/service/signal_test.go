...
import (
	...
	"github.com/maxhubsv/hublive-server/pkg/service/servicefakes"
...
func init() {
	prometheus.Init("node", hublive.NodeType_CONTROLLER)
}
...
func TestSignal(t *testing.T) {
	...
	t.Run("messages are delivered", func(t *testing.T) {
		...
		reqMessageIn := &hublive.SignalRequest{
			Message: &hublive.SignalRequest_Ping{Ping: 123},
		}
		resMessageIn := &hublive.SignalResponse{
			Message: &hublive.SignalResponse_Pong{Pong: 321},
		}
		...
		client, err := routing.NewSignalClient(hublive.NodeID("node0"), bus, cfg)
		...
		server, err := service.NewSignalServer(hublive.NodeID("node1"), "region", bus, cfg, handler)
		require.NoError(t, err)
		...
		_, reqSink, resSource, err := client.StartParticipantSignal(
			context.Background(),
			hublive.RoomName("room1"),
			routing.ParticipantInit{},
			hublive.NodeID("node1"),
		)
		require.NoError(t, err)
