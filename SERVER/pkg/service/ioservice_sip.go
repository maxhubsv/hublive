...
func (s *IOInfoService) matchSIPTrunk(ctx context.Context, trunkID string, call *rpc.SIPCall) (*hublive.SIPInboundTrunkInfo, error) {
	if s.ss == nil {
	...
	if trunkID != "" {
		...
		if tr, err := s.ss.LoadSIPInboundTrunk(ctx, trunkID); err == nil {
			tr, err = sip.MatchTrunkIter(iters.Slice([]*hublive.SIPInboundTrunkInfo{tr}), call)
			if err == nil {
				return tr, nil
			}
		}
...
func (s *IOInfoService) SelectSIPInboundTrunk(ctx context.Context, called string) iters.Iter[*hublive.SIPInboundTrunkInfo] {
	it := hublive.ListPageIter(s.ss.ListSIPInboundTrunk, &hublive.ListSIPInboundTrunkRequest{
...
func (s *IOInfoService) matchSIPDispatchRule(ctx context.Context, trunk *hublive.SIPInboundTrunkInfo, req *rpc.EvaluateSIPDispatchRulesRequest) (*hublive.SIPDispatchRuleInfo, error) {
	if s.ss == nil {
		return nil, ErrSIPNotConnected
	}
	var trunkID string
...
func (s *IOInfoService) SelectSIPDispatchRule(ctx context.Context, trunkID string) iters.Iter[*hublive.SIPDispatchRuleInfo] {
	var trunkIDs []string
	if trunkID != "" {
		trunkIDs = []string{trunkID}
	}
	it := hublive.ListPageIter(s.ss.ListSIPDispatchRule, &hublive.ListSIPDispatchRuleRequest{
		TrunkIds: trunkIDs,
	})
	return iters.PagesAsIter(ctx, it)
}
...
func (s *IOInfoService) GetSIPTrunkAuthentication(ctx context.Context, req *rpc.GetSIPTrunkAuthenticationRequest) (*rpc.GetSIPTrunkAuthenticationResponse, error) {
	...
	providerInfo := &hublive.ProviderInfo{
		Id:   trunk.SipTrunkId,
		Name: trunk.Name,
		Type: hublive.ProviderType_PROVIDER_TYPE_EXTERNAL, // External trunk
	}
