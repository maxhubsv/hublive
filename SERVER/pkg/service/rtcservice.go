...
func (s *RTCService) validateInternal(
	...
	strict bool,
) (hublive.RoomName, routing.ParticipantInit, int, error) {
	...
	if wrappedJoinRequestBase64 == "" {
		...
		if wrappedProtoBytes, err := base64.URLEncoding.DecodeString(wrappedJoinRequestBase64); err != nil {
			...
			wrappedJoinRequest := &hublive.WrappedJoinRequest{}
			...
			case hublive.WrappedJoinRequest_NONE:
			...
			case hublive.WrappedJoinRequest_GZIP:
	...
	pi := routing.ParticipantInit{
		Identity:                hublive.ParticipantIdentity(res.grants.Identity),
		Name:                    hublive.ParticipantName(res.grants.Name),
		Grants:                  res.grants,
		Region:                  res.region,
	...
	if wrappedJoinRequestBase64 == "" {
		...
		pi.ReconnectReason = hublive.ReconnectReason(reconnectReason)

		if pi.Reconnect {
			pi.ID = hublive.ParticipantID(r.FormValue("sid"))
		}
		...
		pi.ID = hublive.ParticipantID(joinRequest.ParticipantSid)
	}
...
func (s *RTCService) serve(w http.ResponseWriter, r *http.Request, needsJoinRequest bool) {
	...
	var initialResponse *hublive.SignalResponse
	...
	if !pi.Reconnect && initialResponse.GetJoin() != nil {
		joinRoomID := hublive.RoomID(initialResponse.GetJoin().GetRoom().GetSid())
		if joinRoomID != "" {
			roomID = joinRoomID
		}

		pi.ID = hublive.ParticipantID(initialResponse.GetJoin().GetParticipant().GetSid())
		pID = pi.ID
	...
	if pi.Reconnect && pi.ID != "" {
		signalStats.ResolveParticipant(&hublive.ParticipantInfo{
			Sid:      string(pi.ID),
	...
	go func() {
		...
		for {
			...
			case msg := <-cr.ResponseSource.ReadChan():
				...
				res, ok := msg.(*hublive.SignalResponse)
				if !ok {
					pLogger.Errorw(
				...
				case *hublive.SignalResponse_RoomUpdate:
					updateRoomID := hublive.RoomID(m.RoomUpdate.GetRoom().GetSid())
					if updateRoomID != "" {
						roomID = updateRoomID
						resolveLogger(false)
					}
				...
				case *hublive.SignalResponse_RoomMoved:
					resetLogger()
					signalStats.Reset()

					roomName = hublive.RoomName(m.RoomMoved.GetRoom().GetName())
					moveRoomID := hublive.RoomID(m.RoomMoved.GetRoom().GetSid())
					if moveRoomID != "" {
						roomID = moveRoomID
					}
					participantIdentity = hublive.ParticipantIdentity(m.RoomMoved.GetParticipant().GetIdentity())
					pID = hublive.ParticipantID(m.RoomMoved.GetParticipant().GetSid())
					resolveLogger(false)
	...
	for {
		...
		case *hublive.SignalRequest_Ping:
			count, perr := sigConn.WriteResponse(&hublive.SignalResponse{
				Message: &hublive.SignalResponse_Pong{
					//
		...
		case *hublive.SignalRequest_PingReq:
			count, perr := sigConn.WriteResponse(&hublive.SignalResponse{
				Message: &hublive.SignalResponse_PongResp{
					PongResp: &hublive.Pong{
...
) (connectionResult, *hublive.SignalResponse, error) {
	var cr connectionResult
	var err error
...
func readInitialResponse(source routing.MessageSource, timeout time.Duration) (*hublive.SignalResponse, error) {
