...
func (s *IOInfoService) CreateIngress(ctx context.Context, info *hublive.IngressInfo) (*emptypb.Empty, error) {
	if s.is == nil {
...
func (s *IOInfoService) loadIngressFromInfoRequest(req *rpc.GetIngressInfoRequest) (info *hublive.IngressInfo, err error) {
	if s.is == nil {
		return nil, ErrIngressNotConnected
	}
...
func (s *IOInfoService) UpdateIngressState(ctx context.Context, req *rpc.UpdateIngressStateRequest) (*emptypb.Empty, error) {
	...
	if info.State.Status != req.State.Status {
		...
		case hublive.IngressState_ENDPOINT_ERROR,
			hublive.IngressState_ENDPOINT_INACTIVE,
			hublive.IngressState_ENDPOINT_COMPLETE:
		...
		case hublive.IngressState_ENDPOINT_PUBLISHING:
