...
type WHIPService struct {
	...
	client            rpc.WHIPClient[hublive.NodeID]
...
func NewWHIPService(
	...
	client, err := rpc.NewWHIPClient[hublive.NodeID](clientParams.Args())
	if err != nil {
		return nil, err
	}
...
func (s *WHIPService) validateCreate(r *http.Request) (*createRequest, int, error) {
	...
	clientInfoHeader := r.Header.Get("X-HubLive-ClientInfo")
	if clientInfoHeader != "" {
	...
	fromIngress := r.Header.Get("X-Hublive-Ingress")
	...
	pi := routing.ParticipantInit{
		Identity:      hublive.ParticipantIdentity(claims.Identity),
		Name:          hublive.ParticipantName(claims.Name),
		AutoSubscribe: true,
		Client:        ci,
		Grants:        claims,
...
func (s *WHIPService) handleCreate(w http.ResponseWriter, r *http.Request) {
	...
	connID := hublive.ConnectionID(guid.New("CO_"))
	...
	res, err := s.client.Create(r.Context(), hublive.NodeID(rtcNode.Id), &rpc.WHIPCreateRequest{
...
func (s *WHIPService) handleParticipantPatch(w http.ResponseWriter, r *http.Request) {
	...
	pID := hublive.ParticipantID(r.PathValue("participant_id"))
	if pID == "" {
	...
	if ifMatch == "*" {
		s.iceRestart(w, r, roomName, hublive.ParticipantIdentity(claims.Identity), pID, sdpFragment)
	} else {
		s.iceTrickle(w, r, roomName, hublive.ParticipantIdentity(claims.Identity), pID, ifMatch, sdpFragment)
	}
}
...
func (s *WHIPService) handleParticipantDelete(w http.ResponseWriter, r *http.Request) {
	...
	_, err = s.participantClient.DeleteSession(
		r.Context(),
		s.topicFormatter.ParticipantTopic(r.Context(), roomName, hublive.ParticipantIdentity(claims.Identity)),
