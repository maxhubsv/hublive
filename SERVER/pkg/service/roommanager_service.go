...
func (s whipService) Create(ctx context.Context, req *rpc.WHIPCreateRequest) (*rpc.WHIPCreateResponse, error) {
	...
	if err = s.RoomManager.StartSession(
		ctx,
		*pi,
		routing.NewNullMessageSource(hublive.ConnectionID(req.StartSession.ConnectionId)), // no requestSource
		routing.NewNullMessageSink(hublive.ConnectionID(req.StartSession.ConnectionId)),   // no responseSink
	...
	room := s.RoomManager.GetRoom(ctx, hublive.RoomName(req.StartSession.RoomName))
	if room == nil {
	...
	if err := lp.HandleOffer(&hublive.SessionDescription{
	...
	for publisherIdentity, trackList := range req.SubscribedParticipantTracks {
		for _, trackName := range trackList.TrackNames {
			eg.Go(func() error {
				for {
					if lp.IsTrackNameSubscribed(hublive.ParticipantIdentity(publisherIdentity), trackName) {
						return nil
					}
...
func getMediaStateForParticipant(participant types.Participant) (*hublive.InputVideoState, *hublive.InputAudioState) {
	pParticipant := participant.ToProto()

	var video *hublive.InputVideoState
	var audio *hublive.InputAudioState
	...
	for _, v := range pParticipant.Tracks {
		...
		if v.Type != hublive.TrackType_VIDEO {
			continue
		}

		video = &hublive.InputVideoState{}
	...
	for _, a := range pParticipant.Tracks {
		...
		if a.Type != hublive.TrackType_AUDIO {
			continue
		}

		audio = &hublive.InputAudioState{}
...
func (r whipParticipantService) ICETrickle(ctx context.Context, req *rpc.WHIPParticipantICETrickleRequest) (*emptypb.Empty, error) {
	room := r.RoomManager.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	lp := room.GetParticipantByID(hublive.ParticipantID(req.ParticipantId))
	if lp == nil {
		return nil, ErrParticipantNotFound
	}
