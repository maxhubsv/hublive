...
func HandshakeAgentWorker(c agent.SignalConn, serverInfo *hublive.ServerInfo, registration agent.WorkerRegistration, l logger.Logger) (r agent.WorkerRegistration, ok bool) {
...
type AgentHandler struct {
	...
	jobToWorker map[hublive.JobID]*agent.Worker
...
func NewAgentHandler(
	...
	return &AgentHandler{
		...
		jobToWorker:      make(map[hublive.JobID]*agent.Worker),
...
func (h *AgentHandler) registerWorker(w *agent.Worker) {
	...
	if created {
		...
		switch w.JobType {
		case hublive.JobType_JT_ROOM:
			typeTopic = h.roomTopic
		case hublive.JobType_JT_PUBLISHER:
			typeTopic = h.publisherTopic
		case hublive.JobType_JT_PARTICIPANT:
...
func (h *AgentHandler) deregisterJob(jobID hublive.JobID) {
...
func (h *AgentHandler) JobRequest(ctx context.Context, job *hublive.Job) (*rpc.JobRequestResponse, error) {
	...
	for {
		...
		case hublive.JobStatus_JS_RUNNING:
			logger.Infow("assigned job to worker")
			h.mu.Lock()
			h.jobToWorker[hublive.JobID(job.Id)] = selected
			h.mu.Unlock()
			...
			fallthrough
		case hublive.JobStatus_JS_SUCCESS:
...
func (h *AgentHandler) JobRequestAffinity(ctx context.Context, job *hublive.Job) float32 {
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	for _, w := range h.workers {
		...
		if w.Status() == hublive.WorkerStatus_WS_AVAILABLE {
...
func (h *AgentHandler) JobTerminate(ctx context.Context, req *rpc.JobTerminateRequest) (*rpc.JobTerminateResponse, error) {
	h.mu.Lock()
	w := h.jobToWorker[hublive.JobID(req.JobId)]
	h.mu.Unlock()
	...
	state, err := w.TerminateJob(hublive.JobID(req.JobId), req.Reason)
	if err != nil {
		return nil, err
	}
...
func (h *AgentHandler) selectWorkerWeightedByLoad(key workerKey, ignore map[*agent.Worker]struct{}) (*agent.Worker, error) {
	...
	for _, w := range workers {
		if _, ok := ignore[w]; !ok && w.Status() == hublive.WorkerStatus_WS_AVAILABLE {
...
func (w *agentHandlerWorker) HandleUpdateJob(update *hublive.UpdateJobStatus) error {
	...
	if agent.JobStatusIsEnded(update.Status) {
		w.h.mu.Lock()
		w.h.deregisterJob(hublive.JobID(update.JobId))
		w.h.mu.Unlock()
	}
	return nil
}
