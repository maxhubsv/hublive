...
func (h *TURNAuthHandler) CreateUsername(apiKey string, pID hublive.ParticipantID) string {
	return base62.EncodeToString([]byte(fmt.Sprintf("%s|%s", apiKey, pID)))
}

func (h *TURNAuthHandler) ParseUsername(username string) (apiKey string, pID hublive.ParticipantID, err error) {
	...
	return parts[0], hublive.ParticipantID(parts[1]), nil
}

func (h *TURNAuthHandler) CreatePassword(apiKey string, pID hublive.ParticipantID) (string, error) {
...
func (h *TURNAuthHandler) HandleAuth(username, realm string, srcAddr net.Addr) (key []byte, ok bool) {
	...
	password, err := h.CreatePassword(parts[0], hublive.ParticipantID(parts[1]))
	if err != nil {
		logger.Warnw("could not create TURN password", err, "username", username)
		return nil, false
	}
	return turn.GenerateAuthKey(username, HubliveRealm, password), true
}
