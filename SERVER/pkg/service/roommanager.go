...
import (
	...
	"__GITHUB_HUBLIVE__mediatransportutil/pkg/rtcconfig"
	...
	"github.com/maxhubsv/hublive-server/pkg/clientconfiguration"
	...
	"github.com/maxhubsv/hublive-server/version"
)
...
type RoomManager struct {
	...
	whipServer        rpc.WHIPServer[hublive.NodeID]
	...
	rooms map[hublive.RoomName]*rtc.Room
...
func NewLocalRoomManager(
	...
	r := &RoomManager{
		...
		rooms: make(map[hublive.RoomName]*rtc.Room),

		iceConfigCache: sutils.NewIceConfigCache[iceConfigCacheKey](0),

		serverInfo: &hublive.ServerInfo{
			Edition:       hublive.ServerInfo_Standard,
	...
	r.whipServer, err = rpc.NewWHIPServer[hublive.NodeID](whipService, bus, rpc.WithDefaultServerOptions(conf.PSRPC, logger.GetLogger()))
	if err != nil {
		return nil, err
	}
...
func (r *RoomManager) GetRoom(_ context.Context, roomName hublive.RoomName) *rtc.Room {
	r.lock.RLock()
...
func (r *RoomManager) deleteRoom(ctx context.Context, roomName hublive.RoomName) error {
...
func (r *RoomManager) CreateRoom(ctx context.Context, req *hublive.CreateRoomRequest) (*hublive.Room, error) {
...
func (r *RoomManager) StartSession(
	...
	if participant != nil {
		...
		if pi.Reconnect {
			if participant.IsClosed() {
				...
				var leave *hublive.LeaveRequest
				pv := types.ProtocolVersion(pi.Client.Protocol)
				if pv.SupportsRegionsInLeaveRequest() {
					leave = &hublive.LeaveRequest{
						Reason: hublive.DisconnectReason_STATE_MISMATCH,
						Action: hublive.LeaveRequest_RECONNECT,
					}
				} else {
					leave = &hublive.LeaveRequest{
						CanReconnect: true,
				...
				_ = responseSink.WriteMessage(&hublive.SignalResponse{
			...
			if err = room.ResumeParticipant(
				...
				r.iceServersForParticipant(
					apiKey,
					participant,
					iceConfig.PreferenceSubscriber == hublive.ICECandidateType_ICT_TLS,
				),
				pi.ReconnectReason,
			); err != nil {
	...
	iceServers := r.iceServersForParticipant(apiKey, participant, iceConfig.PreferenceSubscriber == hublive.ICECandidateType_ICT_TLS)
	...
	persistRoomForParticipantCount := func(proto *hublive.Room) {
	...
	clientMeta := &hublive.AnalyticsClientMeta{Region: r.currentNode.Region(), Node: string(r.currentNode.NodeID())}
	...
	participant.OnICEConfigChanged(func(participant types.LocalParticipant, iceConfig *hublive.ICEConfig) {
...
func (r *RoomManager) getOrCreateRoom(ctx context.Context, createRoom *hublive.CreateRoomRequest) (*rtc.Room, error) {
	roomName := hublive.RoomName(createRoom.Name)
...
func (r *RoomManager) roomAndParticipantForReq(ctx context.Context, req participantReq) (*rtc.Room, types.LocalParticipant, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.GetRoom()))
	if room == nil {
		return nil, nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(hublive.ParticipantIdentity(req.GetIdentity()))
	if participant == nil {
...
func (r *RoomManager) ListParticipants(ctx context.Context, req *hublive.ListParticipantsRequest) (*hublive.ListParticipantsResponse, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participants := room.GetParticipants()
	items := make([]*hublive.ParticipantInfo, 0, len(participants))
	for _, p := range participants {
		items = append(items, p.ToProto())
	}

	return &hublive.ListParticipantsResponse{
		Participants: items,
	}, nil
}

func (r *RoomManager) GetParticipant(ctx context.Context, req *hublive.RoomParticipantIdentity) (*hublive.ParticipantInfo, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(hublive.ParticipantIdentity(req.Identity))
	if participant == nil {
...
func (r *RoomManager) RemoveParticipant(ctx context.Context, req *hublive.RoomParticipantIdentity) (*hublive.RemoveParticipantResponse, error) {
	...
	room.RemoveParticipant(hublive.ParticipantIdentity(req.Identity), "", types.ParticipantCloseReasonServiceRequestRemoveParticipant)
	return &hublive.RemoveParticipantResponse{}, nil
}

func (r *RoomManager) MutePublishedTrack(ctx context.Context, req *hublive.MuteRoomTrackRequest) (*hublive.MuteRoomTrackResponse, error) {
	...
	track := participant.SetTrackMuted(&hublive.MuteTrackRequest{
		Sid:   req.TrackSid,
		Muted: req.Muted,
	}, true)
	return &hublive.MuteRoomTrackResponse{Track: track}, nil
}

func (r *RoomManager) UpdateParticipant(ctx context.Context, req *hublive.UpdateParticipantRequest) (*hublive.ParticipantInfo, error) {
	_, participant, err := r.roomAndParticipantForReq(ctx, req)
	if err != nil {
		return nil, err
	}

	if err = participant.UpdateMetadata(&hublive.UpdateParticipantMetadata{
		Name:       req.Name,
...
func (r *RoomManager) ForwardParticipant(ctx context.Context, req *hublive.ForwardParticipantRequest) (*hublive.ForwardParticipantResponse, error) {
	return nil, errors.New("not implemented")
}

func (r *RoomManager) MoveParticipant(ctx context.Context, req *hublive.MoveParticipantRequest) (*hublive.MoveParticipantResponse, error) {
	return nil, errors.New("not implemented")
}

func (r *RoomManager) PerformRpc(ctx context.Context, req *hublive.PerformRpcRequest) (*hublive.PerformRpcResponse, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.GetRoom()))
	if room == nil {
		return nil, ErrRoomNotFound
	}

	participant := room.GetParticipant(hublive.ParticipantIdentity(req.GetDestinationIdentity()))
	if participant == nil {
		return nil, ErrParticipantNotFound
	}
	...
	case result := <-resultChan:
		return &hublive.PerformRpcResponse{Payload: result}, nil
	case err := <-errorChan:
		return nil, err
	}
}

func (r *RoomManager) DeleteRoom(ctx context.Context, req *hublive.DeleteRoomRequest) (*hublive.DeleteRoomResponse, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		...
		err := r.roomStore.DeleteRoom(ctx, hublive.RoomName(req.Room))
		if err != nil {
	...
	return &hublive.DeleteRoomResponse{}, nil
}

func (r *RoomManager) UpdateSubscriptions(ctx context.Context, req *hublive.UpdateSubscriptionsRequest) (*hublive.UpdateSubscriptionsResponse, error) {
	room, participant, err := r.roomAndParticipantForReq(ctx, req)
	if err != nil {
		return nil, err
	}
	...
	room.UpdateSubscriptions(
		participant,
		hublive.StringsAsIDs[hublive.TrackID](req.TrackSids),
		req.ParticipantTracks,
		req.Subscribe,
	)
	return &hublive.UpdateSubscriptionsResponse{}, nil
}

func (r *RoomManager) SendData(ctx context.Context, req *hublive.SendDataRequest) (*hublive.SendDataResponse, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
	...
	return &hublive.SendDataResponse{}, nil
}

func (r *RoomManager) UpdateRoomMetadata(ctx context.Context, req *hublive.UpdateRoomMetadataRequest) (*hublive.Room, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) ListDispatch(ctx context.Context, req *hublive.ListAgentDispatchRequest) (*hublive.ListAgentDispatchResponse, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
	...
	ret := &hublive.ListAgentDispatchResponse{
...
func (r *RoomManager) CreateDispatch(ctx context.Context, req *hublive.AgentDispatch) (*hublive.AgentDispatch, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) DeleteDispatch(ctx context.Context, req *hublive.DeleteAgentDispatchRequest) (*hublive.AgentDispatch, error) {
	room := r.GetRoom(ctx, hublive.RoomName(req.Room))
	if room == nil {
		return nil, ErrRoomNotFound
	}
...
func (r *RoomManager) iceServersForParticipant(apiKey string, participant types.LocalParticipant, tlsOnly bool) []*hublive.ICEServer {
	var iceServers []*hublive.ICEServer
	...
	if r.config.TURN.Enabled {
		...
		if len(urls) > 0 {
			...
			if err != nil {
				...
				iceServers = append(iceServers, &hublive.ICEServer{
					Urls:       urls,
					Username:   username,
					Credential: password,
				})
...
func (r *RoomManager) setIceConfig(roomName hublive.RoomName, participant types.LocalParticipant) *hublive.ICEConfig {
	iceConfig := r.getIceConfig(roomName, participant)
	participant.SetICEConfig(iceConfig)
	return iceConfig
}

func (r *RoomManager) getIceConfig(roomName hublive.RoomName, participant types.LocalParticipant) *hublive.ICEConfig {
...
func iceServerForStunServers(servers []string) *hublive.ICEServer {
	iceServer := &hublive.ICEServer{}
...
func (h *roomManagerParticipantHelper) GetParticipantInfo(pID hublive.ParticipantID) *hublive.ParticipantInfo {
...
func (h *roomManagerParticipantHelper) GetRegionSettings(ip string) *hublive.RegionSettings {
	return nil
}

func (h *roomManagerParticipantHelper) GetSubscriberForwarderState(lp types.LocalParticipant) (map[hublive.TrackID]*hublive.RTPForwarderState, error) {
	return nil, nil
}

func (h *roomManagerParticipantHelper) ResolveMediaTrack(lp types.LocalParticipant, trackID hublive.TrackID) types.MediaResolverResult {
	return h.room.ResolveMediaTrackForSubscriber(lp, trackID)
}

func (h *roomManagerParticipantHelper) ResolveDataTrack(lp types.LocalParticipant, trackID hublive.TrackID) types.DataResolverResult {
...
func (h *roomManagerParticipantHelper) GetCachedReliableDataMessage(seqs map[hublive.ParticipantID]uint32) []*types.DataMessageCache {
